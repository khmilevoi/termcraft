...
import type { ChatIndexCanonicalSource, ChatIndexPage, ChatIndexPageStore, PageCursor } from "../types"
import {
...
async function buildFor(bytes: Uint8Array, pages: ChatIndexPageStore, projectViewGeneration = VIEW_GENERATION) {
...
describe("build", () => {
  ...
  test("never stores record bodies — only the four indexed fields plus offsets", async () => {
    ...
    expect(Object.keys(entry ?? {}).sort()).toEqual(["changedPages", "offsetEnd", "offsetStart", "recordId", "ts", "turnId"])
  })
})

describe("a 100k-record fixture builds with bounded resident memory", () => {
  test(
    "pages independently in ≤512-entry pages, one page store write per page",
    async () => {
      ...
      for (const dirEntry of state.pages) expect(dirEntry.entryCount).toBeLessThanOrEqual(CHAT_INDEX_PAGE_SIZE)

      pages.readCount = 0
      const loaded = unwrap(await loadChatIndexTail(state, { pages }, makeSource(bytes)))
      expect(loaded.records.map((r) => r.recordId)).toEqual(records.slice(-100).map((r) => r.recordId))
...
describe("resident-page ceiling (§7.1: never more than CHAT_INDEX_MAX_RESIDENT_PAGES pages touched by one call)", () => {
  test("many small single-record appends produce many single-entry pages, and one loadTail call stops at the resident-page cap rather than reading all of them", async () => {
    ...
    expect(loaded.records.map((r) => r.recordId)).toEqual(appendedIds.slice(-CHAT_INDEX_MAX_RESIDENT_PAGES))
...
describe("backward pagination follows byte offsets correctly (§7.3)", () => {
  test("loadTail then repeated loadBefore reconstructs the whole chat in order", async () => {
    ...
    const page2 = unwrap(await loadChatIndexBefore(state, { pages }, source, requireCursor(tail.prevCursor), { limit: 10 }))
    expect(page2.records).toHaveLength(10)
    expect(page2.prevCursor).not.toBeNull()

    const page3 = unwrap(await loadChatIndexBefore(state, { pages }, source, requireCursor(page2.prevCursor), { limit: 10 }))
    expect(page3.records).toHaveLength(5)
    expect(page3.prevCursor).toBeNull() // reached the beginning

    const reassembled = [...page3.records, ...page2.records, ...tail.records].map((r) => r.recordId)
...
describe("a single oversized record is returned alone (§7.3)", () => {
  test("a byte budget smaller than the newest record still returns exactly that record", async () => {
    ...
    const pages = makePageStore()
    const state = unwrap(await buildFor(bytes, pages))
    const loaded = unwrap(await loadChatIndexTail(state, { pages }, makeSource(bytes), { byteBudget: 1024 }))
