...
1. Version creation: one new version file (a page's TSX module) per page changed by an applied turn; every version traces to a chat record via an applied map — the agent record for turns, the system record for rollbacks and UI renames; the head of a page is simply its highest version — no head pointer. Retitling a page edits its own metadata export, so it versions like any content change (from the UI it is a mechanical kernel rewrite of the title literal, recorded as a rename system entry).
...
3. History popup (v1.0): opened with `v` or by clicking the status bar's version segment; lists versions with timestamps and prompt excerpts (found by scanning the project's chats for the record — agent or system — whose applied map names the version); offers explicit rollback; renders over a dimmed backdrop.
4. Rollback semantics: "make v3 the head" copies v3 forward as the new highest version — the git-revert model; auto and explicit rollbacks are recorded as system entries in the active chat, carrying the version they minted in their applied map. Rollback never checks whether the copied source still renders: a rollback may deliberately mint a broken head, and the repair path is the agent itself — sending while the active page's head fails to render asks for confirmation before handing the agent the broken source. Auto-rollback is safe by construction, since the viewed version is being rendered by the live preview.
