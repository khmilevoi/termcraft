...
import { resolveSentPinAppends, type SentPinV1 } from "./turn-resolution"
...
describe("resolveSentPinAppends", () => {
  ...
  test("mints a distinct recordId per resolved pin", () => {
    ...
    const recordIds = result.map((entry) => (entry.event.kind === "pin:status" ? entry.event.recordId : null))
