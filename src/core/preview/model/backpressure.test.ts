...
describe("createPreviewBackpressure", () => {
  ...
  test("draining one below the low-water mark fires exactly one writable event", () => {
    ...
    const releasesBelowLowWaterMark = HOST_CONTROL_QUEUE_CAPACITY - HOST_CONTROL_QUEUE_LOW_WATER_MARK + 1
    ...
    expect(events).toEqual([{ kind: "writable", queueSize: HOST_CONTROL_QUEUE_LOW_WATER_MARK - 1 }])
  })
