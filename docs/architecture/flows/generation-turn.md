...
3. While the turn runs: status streams into chat as ephemeral text, never persisted; typing the next message is allowed but sending stays disabled; version switching, rollback, export, the agent · model · effort picker, and chat creation and switching are locked, each refusal hinted in the status bar. The composer's context-usage indicator updates from token usage the backend reports in its event stream; a backend that reports nothing hides the indicator.
