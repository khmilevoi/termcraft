...
5. Failure branch (design target — no `agent` module or shell exists yet to implement it): if the agent CLI is missing or not logged in, a background health check performed at startup — and repeated before each send — surfaces the problem in the status bar. Attempting to send a message while unhealthy then fails with a clear error and install instructions rather than silently hanging.
...
- `src/store/sandbox/model/staging-store.ts` — the unique fenced per-turn workspace a backend would confine an agent to (step 3)
...
- `src/gate/model/gate.ts` (orchestrator; stages in `import-scan.ts`, `page-contract.ts`, `lints.ts`, `type-check.ts`, `manifest.ts`) — the validation pipeline: import allowlist, page contract, determinism lints, and the injected type-check/manifest/smoke-render ports
...
- `src/host/supervisor/types.ts` (`PreviewHandle`, built inside `supervisor.ts`) — the restart/backoff/circuit-aware preview facade the step-4 claim describes; `src/host/supervisor/model/preview-session.ts`'s separate, non-restart-aware `PreviewSession` facade also exists but has no caller. Neither has a `ui`-module consumer yet
...
- `docs/superpowers/specs/2026-07-13-termcraft-design.md` — §1 overview, §2 non-goals, §3.1 launch and trust, §4 architecture, §6.1 backend abstraction, §6.2 turn protocol: the agent-CLI integration itself (steps 2, 3, 5) has no code yet — no `agent` or Kernel/`core` module exists
