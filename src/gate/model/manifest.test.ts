...
function codes(text: string, present: PageSlug[]): string[] {
  return checkManifestSlice({ manifestText: text, presentSlugs: present }).errors.map(
    (e) => e.code,
  );
}
...
describe("checkManifestSlice", () => {
  ...
  test("rejects a duplicate slug", () => {
    expect(codes(JSON.stringify({ pages: ["home", "home"] }), [slug("home")])).toContain(
      "MANIFEST_DUPLICATE_SLUG",
    );
  });

  test("rejects a manifest entry with no staged page", () => {
    expect(codes(JSON.stringify({ pages: ["home", "ghost"] }), [slug("home")])).toContain(
      "MANIFEST_UNKNOWN_PAGE",
    );
  });

  test("rejects a staged page absent from the manifest", () => {
    expect(codes(JSON.stringify({ pages: ["home"] }), [slug("home"), slug("orphan")])).toContain(
      "MANIFEST_MISSING_PAGE",
    );
  });

  test("rejects an active page not in the list", () => {
    expect(
      codes(JSON.stringify({ pages: ["home"], active: "settings" }), [slug("home")]),
    ).toContain("MANIFEST_ACTIVE_NOT_LISTED");
  });

  test("rejects a non-string active field", () => {
    expect(codes(JSON.stringify({ pages: ["home"], active: 7 }), [slug("home")])).toContain(
      "MANIFEST_SHAPE",
    );
  });
});
