...
const importError: GateError = { kind: "import", code: "FORBIDDEN_IMPORT", message: "import of 'react' is not allowed" }
const droppedIdWarning: GateWarning = { kind: "dropped-id", message: "id 'visits' was present last iteration" }
...
describe("makeGateResult (§6.3 pass/fail rule)", () => {
  ...
  test("a failed contract carries a null descriptor", () => {
    const result = makeGateResult([{ kind: "contract", code: "NON_STATIC_META", message: "meta is not a literal" }], [], null)
    expect(result.ok).toBe(false)
