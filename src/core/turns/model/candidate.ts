...
function toBeforeMap(
  canonicalPages: StagedTurnReadSetV1["canonicalPages"],
): ReadonlyMap<PageSlug, ReadSetFileSnapshotV1 | null> {
  return new Map(canonicalPages.map((entry) => [entry.pageSlug, entry.snapshot]));
}

function toPresentMap(files: readonly StagedFileV1[]): {
  readonly presentSlugs: readonly PageSlug[];
  readonly presentBySlug: ReadonlyMap<PageSlug, FileImage>;
} {
...
function diffPageInventory(
  ...
  for (const pageSlug of allSlugs) {
    ...
    changes.push({
      pageSlug,
      change: before === null ? "added" : "modified",
      sha256: after.sha256,
      size: after.size,
    });
  }
