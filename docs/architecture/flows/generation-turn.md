...
- `src/gate/model/import-scan.ts` — the single `@termcraft/runtime` import-surface allowlist (rejects dynamic import, re-export, and `require`), plus a FATAL dynamic-code rejection (design §5.8) for `eval`/`Function` — a direct call, a bare `eval` reference, and computed-string evasions — skipping JSX children text via `jsx.ts` so a page's own display copy is never mistaken for a reference
- `src/gate/model/jsx.ts` — the JSX open-tag recognizer (`scanOpenTag`/`readHyphenatedName`) shared by `lints.ts` and `import-scan.ts`, plus `computeJsxTextTokenIndices`, which requires a genuine matching closing tag before trusting anything as children text
