...
- `src/gate/model/jsx.ts` — `scanJsx` (fix-pass-3): a small recursive-descent JSX reader built directly on the installed TypeScript scanner's own JSX language variant (`scanJsxToken`/`scanJsxIdentifier`/`scanJsxAttributeValue`), replacing the earlier `scanOpenTag` code-token heuristic outright. Over one pass of the whole source it yields every real element (tag name, `id` presence, position) and every genuine children text range, fail-closed on anything malformed or unterminated — an unclosed tag or a mismatched close contributes nothing, never masking real code as "text". `lints.ts`'s `unpointed-element` lint consumes `scanJsx`'s elements directly; `import-scan.ts`'s `computeJsxTextTokenIndices` maps its text ranges onto the CODE-mode token stream `lexer.ts`'s `tokenize` produces, for the dynamic-code check above. `readHyphenatedName` stays a standalone token-array helper, used only by `lints.ts`'s `dropped-id` extraction (which also has to recognize the non-JSX `id: "x"` object-literal form)
