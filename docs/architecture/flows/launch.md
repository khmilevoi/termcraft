...
flowchart TD
    ...
    formatq -- "version 1 or 2: must migrate" --> migoffer["migrate-80 dialog — pre-Kernel, no Workspace yet"]
    migoffer -- "esc later" --> migdeclined["exit 0; nothing written"]
    migoffer -- "⏎ migrate" --> migrun["verified backup + MigrationTransaction"] --> reopen2["reopen on format 3, second createShell, composer pre-filled with unsent migration draft"] --> wsmount
...
fills exactly as an already-trusted project would — including its preview: the auto-issued
`preview.selectPage` for the active page, dispatched while the project was still untrusted, is
refused `PROJECT_UNTRUSTED` at open time and clears its own request memo so a later retry can
succeed, but `project.setTrust` changes neither the active page nor its descriptor, so nothing
else would ever ask again. The shell's `mirror.project` subscriber (`src/ui/app/model/deps.ts`)
re-issues that SAME request the instant trust flips to `trusted` (defect fix, 2026-08-03) —
without it the preview pane stayed on "preparing preview…" until the designer happened to switch
to a different page, which is the only other thing that asks. `Esc` (`trust-decline`) sets the flag and
...
2. Project discovery has three outcomes: a missing `.termcraft/` opens Home; a
   ...
   through trust; a version-1 OR version-2 project performs the same lease-and-recovery
   sequence, then fails its manifest read with a typed `ManifestMigrationRequiredError`
   (`format_version` below the current 3) — BEFORE trust, and before any Kernel exists
   to build a Workspace around. That failure is not fatal: the composition root
   (`entrypoint/model/create-shell.ts`) catches it and offers the `migrate-80` dialog
   as the ONLY thing the process draws, `⏎ migrate` runs the
   verified-backup-then-`MigrationTransaction` protocol against whichever origin the
   project scanned as, and the project reopens on format 3 through a second, ordinary
   `createShell` call that proceeds through trust exactly like any current-format
   project — with the composer pre-filled as an UNSENT draft naming the migration, never
   an auto-started turn (plan P4 decision D8) — the "posttrust" bulk-offer this diagram
   used to draw never happens, because there is no live path that reaches trust while
   still below format 3. There is no project picker. `esc later` exits the process with one
...
- `src/ui/app/model/deps.ts` — Home's local `prompt` atom and the Workspace's local `composer` atom: two separate pieces of UI state with no carry-over between them; irrelevant to Gap C's fix, since the first turn now starts from the Home text directly at the Kernel, never by pre-filling or copying into the Workspace composer. Also `requestPreviewForActivePage`'s `mirror.project` subscriber (defect fix, 2026-08-03): retries the active page's `preview.selectPage` the instant `trust` flips to `trusted`, since neither the active page nor its descriptor changes when the follow-up `project.setTrust` lands, so nothing else would re-ask after the auto-issued request made while still untrusted was refused `PROJECT_UNTRUSTED`.
...
- `src/store/model/factory.ts` — `openProject`'s existing-project launch sequence (durability pre-flight → lease → `SafeProjectFs` → journal-format gate → recover transactions → format-too-new gate → read `project.toml`/`workspace.local.toml` → orphan-turn scan → open) and `createProject`'s single project-creation transaction (durability pre-flight → create-new `.termcraft/` → mints the format-3 `project.toml`, `.gitignore`, `workspace.local.toml`, the first chat header, the seeded empty `design/pages.json`, and the seeded `design/system/` design-system files from `store/model/design-system-seed.ts` — all in ONE transaction), followed by the implicit trust grant.
...
- `src/store/migration/model/registry.ts` — the format-too-new gate shared by every durable file kind, and the migration chain itself — no longer empty: two steps, `project.toml` format 1 -> 2 (design-tree phase 1b) and 2 -> 3 (plan P4, seeding `design/system/`), both landing a project on format 3 (`flows/migration.md`).
...
- `src/store/toml/model/project-toml.ts` — `project.toml`'s schema (`format_version`, `project_id`, `name`, `created_at`, `target_stack` — the `pages` array retired at format 2, since `design/pages.json` is the sole ordering authority) plus its own format-too-new gate and the `ManifestMigrationRequiredError` an older format raises.
