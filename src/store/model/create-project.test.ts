...
import { encodeChatHeaderLine } from "store/jsonl"
...
describe("createProject — new-project creation (storage-identity §14.2)", () => {
  test("mints projectId, the format-1 layout, the gitignore, the workspace file, the first chat header, and the implicit trust grant", async () => {
    ...
    const opened = await store.createProject({ root: projectRoot, name: "My Project", targetStack: "generic" })
    ...
    try {
      ...
      if (manifest instanceof Error) throw new Error(`fixture bug: manifest unreadable: ${manifest.message}`)
      ...
      const manifestOnDisk = fs.readFileSync(path.join(projectRoot, ".termcraft", PROJECT_MANIFEST_FILENAME), "utf8")
      expect(manifestOnDisk).toBe(encodeProjectManifest(manifest))

      // ---- .gitignore: byte-deterministic generated content ----
      const gitignoreOnDisk = fs.readFileSync(path.join(projectRoot, ".termcraft", PROJECT_GITIGNORE_FILENAME), "utf8")
      expect(gitignoreOnDisk).toBe(renderProjectGitignore())

      // ---- workspace.local.toml: machine-local defaults ----
      const workspaceOnDisk = fs.readFileSync(path.join(projectRoot, ".termcraft", WORKSPACE_STATE_FILENAME), "utf8")
      const decodedWorkspace = decodeWorkspaceLocalState(workspaceOnDisk)
      if (decodedWorkspace instanceof Error) throw new Error(`fixture bug: workspace state undecodable: ${decodedWorkspace.message}`)
      ...
      expect(handle.header).toEqual({ kind: "chat", formatVersion: 1, projectId: manifest.projectId, chatId, createdAt: manifest.createdAt })
      ...
      if (subject instanceof Error) throw new Error(`fixture bug: buildSubject failed: ${subject.message}`)
      ...
      if (slugs instanceof Error) throw new Error(`fixture bug: listSlugs failed: ${slugs.message}`)
  ...
  test("refuses to create a second project over an existing .termcraft directory", async () => {
    ...
    const first = await store.createProject({ root: projectRoot, name: "First", targetStack: "generic" })
    if (first instanceof Error) throw new Error(`fixture bug: ${first.message}`)
    await first.close()

    const second = await store.createProject({ root: projectRoot, name: "Second", targetStack: "generic" })
...
function projectCreationPlan(transactionId: string): { readonly plan: TransactionPlan; readonly payloads: readonly ChildPayload[] } {
  const projectId = uuidv7()
  const chatId = uuidv7()

  const manifestBytes = bytesOf(encodeProjectManifest({ formatVersion: 1, projectId, name: "Crash Sweep Project", createdAt: TS, targetStack: "generic", pages: [] }))
  ...
  const chatHeaderLine = encodeChatHeaderLine({ kind: "chat", formatVersion: 1, projectId, chatId, createdAt: TS })
  if (chatHeaderLine instanceof Error) throw new Error(`fixture bug: ${chatHeaderLine.message}`)

  function op(index: number, target: string, bytes: Uint8Array, payloadId: string): TransactionOperation {
    return { index, target, mode: "replace", oldImage: { state: "absent" }, newImage: { state: "file", sha256: sha256Hex(bytes), size: bytes.byteLength }, payloadId }
  }
...
const SWEPT_BOUNDARIES: readonly TransactionBoundary[] = ["plan-installed", "intent-installed", "operation-target-applied", "committed-installed"]
