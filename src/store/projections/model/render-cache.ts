...
import type { AbsPath, ExportRenderKey, ProjectionFsDeps, RenderEntry, RenderPinPredicate, Sha256Hex } from "../types"
...
function sortedFlagEntries(flags: Readonly<Record<string, boolean | number | string>>): readonly (readonly [string, boolean | number | string])[] {
...
function encodeBuffer(bytes: Uint8Array) {
  return { base64: Buffer.from(bytes).toString("base64"), byteLength: bytes.byteLength, sha256: sha256Hex(bytes) }
}
...
export const nodeRenderCacheFsDeps: ProjectionFsDeps = {
  readFile(absPath) {
    const bytes = errore.try({
      ...
      catch: (cause) => new RenderCacheIoError({ operation: "read", path: absPath, detail: String(cause), cause }),
    })
  ...
  remove(absPath) {
    return errore.try({
      ...
      catch: (cause) => new RenderCacheIoError({ operation: "remove", path: absPath, detail: String(cause), cause }),
    })
  },
  listFiles(absDir) {
    const names = errore.try({
      try: () => fs.readdirSync(absDir),
      catch: (cause) => new RenderCacheIoError({ operation: "list", path: absDir, detail: String(cause), cause }),
    })
  ...
  ensureDir(absDir) {
    return errore.try({
      ...
      catch: (cause) => new RenderCacheIoError({ operation: "mkdir", path: absDir, detail: String(cause), cause }),
    })
  },
}
...
function scanEntries(root: AbsPath, fsDeps: ProjectionFsDeps): RenderCacheIoError | readonly ScannedEntry[] {
  const names = fsDeps.listFiles(root)
  if (names instanceof Error) return new RenderCacheIoError({ operation: "list", path: root, detail: names.message, cause: names })
  ...
  for (const name of names) {
    ...
    if (bytes instanceof Error) {
      ...
      console.warn("render-cache: quota sweep skipped an unreadable entry:", entryFile, bytes.message)
      continue
    }
    if (bytes === null) continue // vanished between list and read — not this sweep's problem

    const parsed = errore.try({ try: () => JSON.parse(new TextDecoder().decode(bytes)) as unknown, catch: (cause) => new Error("not valid JSON", { cause }) })
    ...
    if (decoded === null || !decoded.success) {
      ...
      if (removed instanceof Error) console.warn("render-cache: corrupt entry cleanup failed:", removed.message)
      continue
    }
    entries.push({ path: entryFile, key: decoded.data.key, writtenAt: decoded.data.writtenAt, byteLength: bytes.byteLength })
  }
  return entries
}
...
export function createRenderCache(deps: RenderCacheDeps): RenderCache {
  ...
  return {
    async get(key) {
      ...
      if (bytes instanceof Error) return new RenderCacheIoError({ operation: "read", path: target, detail: bytes.message, cause: bytes })
      ...
      if (!decoded.success) {
        console.warn("render-cache: entry ignored (miss):", target, "does not match the envelope schema")
        return null
      }
      const envelope = decoded.data

      if (envelope.storeGeneration !== generation) {
        console.warn("render-cache: entry ignored (miss):", target, `store generation ${envelope.storeGeneration} !== ${generation}`)
        return null
      }
      if (!sameKey(envelope.key, key)) {
        console.warn("render-cache: entry ignored (miss):", target, "key mismatch (hash collision or tampering)")
        return null
      }
      ...
      if (styledFrame === null || textFrame === null || layout === null) {
        console.warn("render-cache: entry ignored (miss):", target, "a buffer failed its length/checksum verification")
        return null
      }
    ...
    async put(entry) {
      ...
      if (created instanceof Error) return new RenderCacheIoError({ operation: "mkdir", path: deps.root, detail: created.message, cause: created })
      ...
      if (wrote instanceof Error) return new RenderCacheIoError({ operation: "write", path: target, detail: wrote.message, cause: wrote })
