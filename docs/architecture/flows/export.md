...
3. Export sizes are a fixed ladder: the page's declared minimum size always, plus 120×30 and 160×40; a standard size is included only when it is at least the minimum on both axes (a page is never rendered below its minimum), and duplicates collapse. Several sizes turn resize behavior into data — the frames show which regions stretch and which stay fixed, so the implementing agent never simulates the design's layout engine.
