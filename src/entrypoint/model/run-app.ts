...
export async function runApp(options: RunAppOptions): Promise<AppStartupError | RunningApp> {
  ...
  const root = await createUiRoot({
    ...
    requestExit,
    // `seedComposerText` is OMITTED, not set to `undefined`, matching `projectOpenPayloadSchema`'s
    // own strict-object precedent a couple of blocks below (design-systems §9).
