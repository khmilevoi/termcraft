...
import type { AbsPath, DiagnosticsEntry, DiagnosticsKey, DiagnosticsPinPredicate, ProjectionFsDeps, Sha256Hex } from "../types"
...
export const nodeDiagnosticsStoreFsDeps: ProjectionFsDeps = {
  readFile(absPath) {
    const bytes = errore.try({
      ...
      catch: (cause) => new DiagnosticsStoreIoError({ operation: "read", path: absPath, detail: String(cause), cause }),
    })
  ...
  remove(absPath) {
    return errore.try({
      ...
      catch: (cause) => new DiagnosticsStoreIoError({ operation: "remove", path: absPath, detail: String(cause), cause }),
    })
  },
  listFiles(absDir) {
    const names = errore.try({
      try: () => fs.readdirSync(absDir),
      catch: (cause) => new DiagnosticsStoreIoError({ operation: "list", path: absDir, detail: String(cause), cause }),
    })
  ...
  ensureDir(absDir) {
    return errore.try({
      ...
      catch: (cause) => new DiagnosticsStoreIoError({ operation: "mkdir", path: absDir, detail: String(cause), cause }),
    })
  },
}
...
function scanEntries(root: AbsPath, fsDeps: ProjectionFsDeps): DiagnosticsStoreIoError | readonly ScannedEntry[] {
  const names = fsDeps.listFiles(root)
  if (names instanceof Error) return new DiagnosticsStoreIoError({ operation: "list", path: root, detail: names.message, cause: names })
  ...
  for (const name of names) {
    ...
    if (bytes instanceof Error) {
      ...
      console.warn("diagnostics-store: quota sweep skipped an unreadable entry:", entryFile, bytes.message)
      continue
    }
    ...
    if (decoded === null || !decoded.success) {
      ...
      if (removed instanceof Error) console.warn("diagnostics-store: corrupt entry cleanup failed:", removed.message)
      continue
    }
...
export function createDiagnosticsStore(deps: DiagnosticsStoreDeps): DiagnosticsStore {
  ...
  return {
    async get(key) {
      ...
      if (bytes instanceof Error) return new DiagnosticsStoreIoError({ operation: "read", path: target, detail: bytes.message, cause: bytes })
      ...
      if (!decoded.success) {
        console.warn("diagnostics-store: entry ignored (miss):", target, "does not match the envelope schema")
        return null
      }
      const envelope = decoded.data

      if (envelope.storeGeneration !== generation) {
        console.warn("diagnostics-store: entry ignored (miss):", target, `store generation ${envelope.storeGeneration} !== ${generation}`)
        return null
      }
      if (!sameKey(envelope.key, key)) {
        console.warn("diagnostics-store: entry ignored (miss):", target, "key mismatch (hash collision or tampering)")
        return null
      }
    ...
    async put(entry) {
      ...
      if (created instanceof Error) return new DiagnosticsStoreIoError({ operation: "mkdir", path: deps.root, detail: created.message, cause: created })

      const value = { schemaVersion: entry.schemaVersion, provenance: entry.provenance, observedAt: entry.observedAt, diagnostics: entry.diagnostics }
      const envelope = {
      ...
      if (wrote instanceof Error) return new DiagnosticsStoreIoError({ operation: "write", path: target, detail: wrote.message, cause: wrote })
