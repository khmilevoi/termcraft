...
const FAILURE: FailureDtoV1 = { code: "GIT_COMMAND_FAILED", retryable: true, safeMessage: "git failed", details: {} }
...
describe("createFakeGitHistory", () => {
  ...
  test("inspectProject() honors a configured project state", async () => {
    const history = createFakeGitHistory({ project: { repository: "normal", headCommitId: "abc123" } })
    const result = await history.inspectProject("C:/proj")
