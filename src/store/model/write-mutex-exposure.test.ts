...
describe("OpenProject.writeMutex — the same exclusion primitive the engine uses internally (blocker B2)", () => {
  test("a permit held through the public handle blocks a TransactionEngine call, and releasing it lets that call proceed", async () => {
    ...
    const opened = await store.createProject({ root: projectRoot, name: "Mutex Test", targetStack: "generic" })
    ...
    try {
      ...
      const enginePromise = opened.transactions
        .runProjectMutation(emptyMutation({ transactionId: "0190fc4a-8b5c-7d3e-8a91-6f2e4c7b5d01", actionId: "0190fc4a-8b5c-7d3e-8a91-6f2e4c7b5d02", createdAt: "2026-07-21T10:00:00Z" }))
        .then((result) => {
  ...
  test("openProject also exposes the shared mutex — a permit held after reopening blocks a fresh engine call the same way", async () => {
    ...
    const created = await store.createProject({ root: projectRoot, name: "Reopen Mutex Test", targetStack: "generic" })
    ...
    if (reopened instanceof Error) throw new Error(`fixture bug: openProject failed: ${reopened.message}`)
    ...
    try {
      ...
      const enginePromise = reopened.transactions
        .runProjectMutation(emptyMutation({ transactionId: "0190fc4a-8b5c-7d3e-8a91-6f2e4c7b5d03", actionId: "0190fc4a-8b5c-7d3e-8a91-6f2e4c7b5d04", createdAt: "2026-07-21T10:00:00Z" }))
        .then((result) => {
          order.push("engine-settled")
          return result
        })
