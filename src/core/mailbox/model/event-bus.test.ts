...
import { type EventEnvelopeV1, type PublishableEventV1, createEventBus } from "./event-bus";
...
function selectionEvent(
  correlation?: PublishableEventV1["correlation"],
...
describe("createEventBus", () => {
  ...
  test("several events from one transition share one stateRevision", () => {
    context.start(() => {
      ...
      const envelopes = unwrap(
        bus.publishTransition([selectionEvent(), selectionEvent(), selectionEvent()]),
      );
