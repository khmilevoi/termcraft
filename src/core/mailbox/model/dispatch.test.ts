...
import { createEventBus, type EventEnvelopeV1 } from "./event-bus"
import type { ActiveTurnCancelState, TurnCancelProbe } from "./revision-guard"

import { createDispatch, type Dispatch, type DispatchDeps } from "./dispatch"
 ...
 * their job.
 */
function setup(overrides: Partial<DispatchDeps> = {}): { deps: DispatchDeps; dispatch: Dispatch["dispatch"] } {
...
function expectAccepted(result: DispatchResult): asserts result is Extract<CommandResultV1, { status: "accepted" }> {
  if (result instanceof Error) throw result
  if (result.status !== "accepted") throw new Error(`expected accepted, got ${JSON.stringify(result)}`)
}

function expectRejected(result: DispatchResult): asserts result is Extract<CommandResultV1, { status: "rejected" }> {
  if (result instanceof Error) throw result
  if (result.status !== "rejected") throw new Error(`expected rejected, got ${JSON.stringify(result)}`)
}
...
describe("createDispatch — §12.1 step 2: decode before dedupe", () => {
  ...
  test("a malformed envelope never poisons the dedupe ledger for its commandId", async () => {
    ...
    const badResult = await dispatch(makeRawCommand({ commandId, kind: "selection.clear", payload: { bogus: true } }))
    expect(badResult).toBeInstanceOf(CommandDecodeError)

    const goodResult = await dispatch(makeRawCommand({ commandId, kind: "selection.clear", payload: {} }))
...
describe("createDispatch — §12.1 step 3: guard failure rejects without a model transition", () => {
  ...
  test("extractTarget and evaluateGuard receive the decoded kind/payload and the injected state", async () => {
    ...
    expect(evaluateGuard.mock.calls[0]).toEqual(["selection.clear" as CommandKindV1, sentinelTarget, state])
  })
})
...
describe("createDispatch — §12.1 steps 4-6: transition, revision, and publication", () => {
  ...
  test("a no-op handler outcome carrying events logs a warning and still discards them", async () => {
    ...
    try {
      const { dispatch } = setup({
        handle: () => ({ disposition: "no-op", events: [{ kind: "selection.changed", payload: null }] }),
      })
...
describe("createDispatch — §8.5 dedupe: exactly-once end to end", () => {
  ...
  test("a throwing handler returns a value and does not poison the commandId forever", async () => {
    ...
    broken = false
    const recovered = await dispatch(makeRawCommand({ expectedRevision: "0", commandId: uuidv7() }))
