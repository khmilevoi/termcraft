...
function reject(raw: string): PathRuleError {
  ...
  if (!(result instanceof PathRuleError))
...
function accept(raw: string): readonly string[] {
  ...
  if (result instanceof Error)
    throw new Error(`expected acceptance for ${JSON.stringify(raw)}: ${result.message}`);
  return result;
}
...
describe("detectNameCollisions — §5.1 NFC + case-fold collisions", () => {
  ...
  test("rejects two names that differ only by Unicode normalization", () => {
    ...
    expect(detectNameCollisions("pages", [decomposed, precomposed])).toBeInstanceOf(
      NameCollisionError,
    );
  });
