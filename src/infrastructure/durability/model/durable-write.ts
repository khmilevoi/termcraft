...
export function durableFileWrite(
  ...
  const tmpPath = path.join(
    dir,
    `.tmp-${path.basename(targetPath)}-${process.pid}-${crypto.randomBytes(4).toString("hex")}`,
  );
  ...
  if (!Buffer.from(bytes).equals(confirmed))
    return new DurableWriteError({ path: targetPath, step: "confirm" });
