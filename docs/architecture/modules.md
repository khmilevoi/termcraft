...
| **Agent gateway** | Backends over vendors' official TypeScript SDKs: start, stream, confirmed process-tree cancel, health-check; normalization into backend-neutral events; per-chat session checkpoints; per-backend confinement to a unique turn workspace; fencing by turn, attempt, and lease nonce. Built today, in two tiers: a vendor-blind port plus a backend-agnostic shared tier (session-scope derivation, staging containment, the deny-by-default confinement rule), and one vendor tier — the Claude Code backend — behind it. A second vendor (Codex, v1.0) becomes a sibling of that tier supplying its own tool vocabulary, not a change to the shared one. Each attempt runs inside an owned OS process tree, and every terminal outcome resolves only after that tree is confirmed exited. What is still missing is above the gateway, not inside it: no Kernel drives a turn, chooses resume-versus-fresh, or consumes the normalized events, and no composition root selects a backend. |
...
| **HostSupervisor** | Owns the isolated design-host subprocess and versioned length-prefixed protocol for preview, Gate smoke, export, and historical snapshots. `PreviewSession` exposes bounded latest-frame delivery, resize/set-mode/close, and request timeouts; UI never owns the process. `retry()` is a no-op stub, and the restart budget/circuit breaker sit behind the still-uncalled supervisor-level `PreviewHandle`, not `PreviewSession` itself. Ordered input forwarding, geometry queries, and tweak/theme control are part of the target `PreviewSession` contract but not yet implemented — today's facade deliberately omits them. |
...
**Agent gateway — `src/agent/` (port + shared tier) and `src/agent/claude/` (the one vendor tier):**
...
- `src/agent/model/confinement.ts` — the backend-agnostic deny-by-default rule behind the tool veto; parameterized over a tool-name table so it holds no vendor names itself
- `src/agent/model/path-containment.ts` — the staging-containment test: normalize, boundary-safe prefix compare, relative paths resolved against the workspace (not termcraft's own cwd), and a reparse-point check on every segment from the workspace down to the target
- `src/agent/model/session-scope.ts` — the opaque per-`(backend, account, model, workspace)` scope string the store checkpoint is keyed by; effort is deliberately excluded so changing effort never invalidates a session
- `src/agent/claude/model/tool-tables.ts` — Claude Code's own tool vocabulary fed into that shared rule: which file tools require a path, which default to the working directory, and which tools are denied outright
- `src/agent/claude/model/query-fn.ts` — the per-attempt SDK options: workspace as cwd and only writable root, no external settings sources, the deny-by-default veto, and the spawn hook that makes the CLI ours to own
- `src/agent/claude/model/spawn-adopt.ts` — spawn-then-adopt into the owned tree, shared verbatim by a real turn and the health probe, with the membership re-read that turns "adopted" into a verified fact
- `src/agent/claude/model/agent-run.ts` — one attempt end to end: the event stream, the single terminal-outcome latch that decides between natural completion and cancellation, and the exit-confirmation/cancel ladder
- `src/agent/claude/model/claude-backend.ts` — the backend instance: per-run tree ownership, closing the tree on every terminal outcome, and the sticky unhealthy latch an unconfirmed exit sets
- `src/agent/claude/model/health.ts` — the health probe: a minimal isolated query classified into installed / not-logged-in / ready, bounded by a deadline, never throwing
- `src/agent/claude/model/normalize.ts` — vendor messages into `AgentEvent`s (reasoning, tool, final, error, usage), dropping anything with no mapping
...
- `src/host/render/` — the headless OpenTUI renderer and its span-row/color/attribute mapping to the wire format
- `src/host/session/model/host-state-machine.ts` — the in-child protocol state machine; accepts only `mount`/`shutdown` before `ready` and `resize`/`set-mode`/`ping`/`shutdown` after — no `key`/`click`/`query-*`/`set-tweak` kinds exist yet
...
- `src/host/supervisor/model/session.ts` — `createHostSession`: one incarnation end-to-end (spawn → negotiate → mount → ready → pump → resize/setMode/ping → stop)
- `src/host/supervisor/model/supervisor.ts` — `createHostSupervisor`: the multi-session owner (restart/backoff/circuit, ≤10 global host cap, bounded start queue)
- `src/host/supervisor/model/preview-session.ts` — `createPreviewSession`, the UI-facing facade over one `HostSession`; exposes only `identity`/`mode`/`interactionMode`/`frames`/`resize`/`setMode`/`retry`/`close` — its own doc comment lists `forwardInput`, `setTheme`, `setCapabilities`, geometry query, and tweaks as "intentionally absent," and `retry()` is a no-op stub (the separate, restart-aware `PreviewHandle` lives only in `supervisor.ts` and has no caller yet either)
- `src/host/supervisor/model/control-queue.ts`, `control-mailbox.ts` — the bounded ordered outbound/inbound control primitives; built and unit-tested but not called from `session.ts`/`supervisor.ts`'s live path yet
...
- `docs/superpowers/specs/2026-07-16-host-supervision-protocol-design.md` — the still-unbuilt `PreviewSession` surface (input forwarding, geometry query, tweaks/theme control) and the conformant `capture`/layout-tree export reply, neither of which exists in code yet
