...
import { decodeClientHello, decodeHostHello, encodeClientHello, encodeHostHello } from "./hello";
...
describe("client hello", () => {
  ...
  test("rejects a wrong framingVersion literal", () => {
    expect(decodeClientHello(encode({ ...clientHello, framingVersion: 2 }))).toBeInstanceOf(
      ProtocolError,
    );
  });

  test("rejects a wrong kind", () => {
    expect(decodeClientHello(encode({ ...clientHello, kind: "host.hello" }))).toBeInstanceOf(
      ProtocolError,
    );
  });
  ...
  test("rejects a non-[1] offered array", () => {
    expect(
      decodeClientHello(encode({ ...clientHello, offeredProtocolVersions: [1, 2] })),
    ).toBeInstanceOf(ProtocolError);
  });

  test("rejects an unknown mode", () => {
    expect(decodeClientHello(encode({ ...clientHello, mode: "weird" }))).toBeInstanceOf(
      ProtocolError,
    );
  });

  test("rejects a malformed nonce", () => {
    expect(decodeClientHello(encode({ ...clientHello, nonce: "SHORT" }))).toBeInstanceOf(
      ProtocolError,
    );
  });

  test("rejects a source hash of the wrong length", () => {
    expect(decodeClientHello(encode({ ...clientHello, sourceHash: "abc" }))).toBeInstanceOf(
      ProtocolError,
    );
  });

  test("rejects an unknown field", () => {
    expect(decodeClientHello(encode({ ...clientHello, extra: true }))).toBeInstanceOf(
      ProtocolError,
    );
  });
});
...
describe("host hello", () => {
  ...
  test("rejects a wrong selected framing version", () => {
    expect(decodeHostHello(encode({ ...hostHello, selectedFramingVersion: 2 }))).toBeInstanceOf(
      ProtocolError,
    );
  });
});
