...
function mustEncode(messageClass: "control" | "data", payload: Uint8Array): Uint8Array {
...
describe("FrameDecoder", () => {
  ...
  test("emits several frames arriving in one chunk", () => {
    ...
    if (frames instanceof Error) throw frames;
    expect(frames.map((frame) => frame.messageClass)).toEqual(["control", "data", "control"]);
  ...
  test("rejects N above the global ceiling from the length prefix alone", () => {
    ...
    new DataView(header.buffer).setUint32(0, GLOBAL_PAYLOAD_CEILING_BYTES + 1, false);
    const decoder = new FrameDecoder();
    expect(decoder.feed(header)).toBeInstanceOf(FramingError);
  });
  ...
  test("stays poisoned after a violation", () => {
    ...
    new DataView(bad.buffer).setUint32(0, GLOBAL_PAYLOAD_CEILING_BYTES + 1, false);
    const decoder = new FrameDecoder();
