...
export function computePageDescriptorChanges(
  ...
  const beforeBySlug = new Map(before.map((descriptor, index) => [descriptor.pageSlug, { descriptor, index }]))
  const afterBySlug = new Map(after.map((descriptor, index) => [descriptor.pageSlug, { descriptor, index }]))

  const changes: PageDescriptorChangeV1[] = []

  for (const [pageSlug, { descriptor }] of beforeBySlug) {
    if (afterBySlug.has(pageSlug)) continue
    changes.push({ pageSlug, kind: "removed", beforeSourceHash: descriptor.sourceHash, afterSourceHash: null })
  }
  ...
  for (const [pageSlug, { descriptor: afterDescriptor, index: afterIndex }] of afterBySlug) {
    ...
    if (beforeEntry === undefined) {
      changes.push({ pageSlug, kind: "added", beforeSourceHash: null, afterSourceHash: afterDescriptor.sourceHash })
      continue
    }
...
export function buildPageDescriptorsChangedPayload(
  ...
  if (duplicateBefore !== null) {
    return new PageDescriptorsAssemblyError({ reason: `duplicate pageSlug "${duplicateBefore}" in the before list` })
  }
  const duplicateAfter = findDuplicateSlug(after)
  if (duplicateAfter !== null) {
    return new PageDescriptorsAssemblyError({ reason: `duplicate pageSlug "${duplicateAfter}" in the after list` })
  }
