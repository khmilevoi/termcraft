...
describe("tokenize — the over-fire proof, re-measured on every run", () => {
  test("every one of the repository's own .ts/.tsx files tokenizes, with zero refusals", () => {
    ...
    // its test — two branches, two files each, taking it to 947. manifest-and-gate's six tasks
    // add the `entities/design-system` module (`types.ts`, `index.ts`, `model/manifest.ts`,
    // `model/components.ts` and their tests), `gate/model/design-system.ts` and its test, and
    // `gate/model/exports-scan.ts` and its test — taking it to 958.
