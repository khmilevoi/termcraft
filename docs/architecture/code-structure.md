...
command handler registry answering all 48 command kinds (43 → 44, chat-scroll spec
§6.1: `chat.load-older`; 44 → 48, project-design-systems §8.1/§10.1 Wave 3 / P10:
`designSystem.list`/`preview`/`install`/`publish`) — and the production
...
- `src/core/ports/agent-prompt.ts` — `AgentPromptContextV1`/`AgentPromptSource`
  ...
  already documents
- `src/core/ports/design-system-source.ts` — `DesignSystemSource`: `list`/`fetch`/`publish`, asynchronous and failable from the start because "a synchronous contract has no room for a network source later" (§8.1). `list` returns SUMMARIES, not packages, so opening a picker against a remote does not download it; `canPublish` is declared rather than assumed. §8.1's `SourceError` is `FailureDtoV1` at this ring's boundary (decision C1). `src/core/ports/fakes/design-system-source.ts` is its in-memory fake
- `src/core/ports/design-system-install.ts` — P10's two ports keeping `core/design-systems` free of any `store` import: `DesignSystemQuarantinePort` (`admit`/`discard`, over `store/design-systems`' quarantine) and `DesignSystemInstallPort` (`install`/`encodeProvenance`/`readProvenance`, over `TransactionEngine.installDesignSystem` and the provenance codec). `src/store/adapters/design-system-install.ts` is the production adapter pair; see [flows/design-system-install.md](flows/design-system-install.md)
- `src/core/design-systems/` — P10's install pipeline (§8.3–§8.5, decisions D5–D11): `model/candidate.ts`'s `composeDesignSystemCandidate` (splices an incoming package's `system/**` over the canonical tree index, in memory) and `summarizeGatePass` (classifies the Gate's whole-tree answer into a `clean`/`breaks-pages`/`blocked` preview, decision D6); `model/sources.ts`'s `listGrantedSources` (grant-gated, per-source 3 s bound, decision D9/D10) and `detectDesignSystemUpdate`; `model/install.ts`'s `prepareDesignSystemInstall`/`commitDesignSystemInstall`/`discardPreparedInstall`, the trust → fetch → quarantine → candidate → Gate → preview → commit orchestration. No direct I/O anywhere in this module — every effect crosses a port. See [flows/design-system-install.md](flows/design-system-install.md) for the full pipeline
- `src/core/kernel/model/handlers/design-system.ts` — the `designSystem.list`/`preview`/`install`/`publish` command handlers (P10 task 9), following `preview-export.ts`'s `handleExportStart` shape; `designSystemPreview` publishes `designSystem.previewStarted` BEFORE awaiting the freezing `runTree` call (decision D7), and `context.designSystemLedger` is what makes `designSystem.install` a commit of an already-previewed candidate rather than a silent re-preparation
...
- `src/gate/model/import-scan.ts` — the saved-page import allowlist (item 11's last
  ...
  boundaries and once linearly, and unions the findings, because neither reading
...
- `src/store/toml/model/gitignore.ts` — the only Git-adjacent code that exists
  ...
  planner
- `src/store/design-systems/` — the per-user design-system library under `{userStateRoot}/design-systems/` and the one stage-1 source, a local directory: `model/layout.ts` (paths), `model/content-hash.ts` (the domain-separated sha256 over a package's file set), `model/sources-config.ts` (`sources.json`, with the built-in `local` source never configurable away), `model/cache-entry.ts`, `model/summary.ts` (a minimal, NON-EXECUTING manifest read — the §7 fatals belong to the Gate, not here — projected off `entities/design-system`'s own `decodeDesignSystemManifest`, the same decoder the Gate uses, since the P2/P3 reconciliation sync point; a candidate manifest failing that decoder's schema now simply has nothing to list, rather than summarizing and deferring the judgment to the Gate), `model/walk.ts` (the two-phase admission boundary a real safe-fs budget attaches to), and `model/{list,fetch,publish,local-source}.ts`. `list` opens exactly one file per candidate and never a `.tsx`. `src/store/adapters/design-system-source.ts` is the `core/ports/design-system-source.ts` port adapter over this module, with its own `SourceError` → `FailureDtoV1` mapping (`store/adapters/failure.ts`). P10 (decisions D2, D4) added `model/admission.ts` (`createDesignSourceAdmission`, the real `PackageAdmission` over `store/safe-fs`'s `createLimitBudget("workspace")`) and fixed M5 in `fetch.ts` (no-follow at the package ROOT: `fetchLocalPackage` now lists `localLibraryDir`'s parent and applies the same symlink-refusal predicate `list.ts` already applied to the `<id>` entry, rather than following a symlinked `local/<id>` through `listDir`). P10 (decision D3) also fixed M11 in `publish.ts`: the destructive `removeDir` → `renameDir` window is now a three-step swap (`target → .retiring-<id>`, `staging → target`, remove `.retiring-<id>`) with a pre-flight sweep that repairs a crash between steps 1 and 2 on the next publish, so a crash never destroys a previously-published system. `model/quarantine.ts` (P10, decision D4) and `model/provenance.ts` (P10, decision D1) are documented in `docs/architecture/storage.md` item 16 and [flows/design-system-install.md](flows/design-system-install.md)
