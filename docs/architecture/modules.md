...
- `src/gate/model/gate.ts` — the orchestrator (`runGate`): always runs the import scan, page contract, and all five warning lints against the source; `typeCheck`/`checkManifest`/`smokeRender` are optional injected ports, unwired until a composition root exists. `GateInput` carries two further optional fields — `referencedIds` (feeds `dropped-id`) and `listedSlugs` (feeds `unlisted-navigation`) — and each lint skips silently when its field is absent, so the gate still runs standalone with no kernel/manifest behind it
