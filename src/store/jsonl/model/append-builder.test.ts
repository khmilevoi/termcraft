...
describe("buildChatAppend", () => {
  test("produces the §4.4 descriptor and the exact append bytes", () => {
    const built = unwrap(
      buildChatAppend({ before, records: [userRecord(2), userRecord(3)], deps: fakeDeps() }),
    );
    const expected = concat([
      unwrap(encodeChatRecordLine(userRecord(2))),
      unwrap(encodeChatRecordLine(userRecord(3))),
    ]);
  ...
  test("carries an optional domain identity when one is supplied", () => {
    const built = unwrap(
      buildChatAppend({
        before,
        records: [userRecord(2)],
        domainIdentity: TURN_ID,
        deps: fakeDeps(),
      }),
    );
  ...
  test("a duplicate recordId inside one batch is refused before any preparation", () => {
    const rejected = buildChatAppend({
      before,
      records: [userRecord(2), userRecord(2, "again")],
      deps: fakeDeps(),
    });
  ...
  test("a schema-invalid record is refused by the entity decoder", () => {
    const rejected = buildChatAppend({
      before,
      records: [{ ...userRecord(2), ts: "yesterday" } as ChatRecord],
      deps: fakeDeps(),
    });
    expect(rejected).toBeInstanceOf(Error);
  ...
  test("a malformed before-state is refused", () => {
    expect(
      buildChatAppend({
        before: { length: -1, prefixSha256: before.prefixSha256 },
        records: [userRecord(2)],
        deps: fakeDeps(),
      }),
    ...
    expect(
      buildChatAppend({
        before: { length: 10, prefixSha256: "NOTAHASH" },
        records: [userRecord(2)],
        deps: fakeDeps(),
      }),
    ).toBeInstanceOf(Error);
...
describe("computeAfterImage", () => {
  test("hashes the recorded before prefix followed by the append", () => {
    ...
    const image = unwrap(
      computeAfterImage({ chunks: [prefix], beforeLength: before.length, append: built.bytes }),
    );
