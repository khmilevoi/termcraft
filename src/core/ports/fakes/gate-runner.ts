...
import type { GateRunner, GateRunResultV1, ManifestSliceResultV1 } from "../gate-runner"
...
export function createFakeGateRunner(): FakeGateRunner {
  ...
  async function runManifestSlice(input: { manifestText: string; presentSlugs: readonly PageSlug[] }): Promise<ManifestSliceResultV1> {
  ...
  async function runPage(input: { source: string; slug: PageSlug; fileName?: string }): Promise<GateRunResultV1> {
