...
const HOME_BYTES = new TextEncoder().encode("export const meta = {} \n export default function Home() {}\n")
...
function renderResult(overrides: Partial<ExportRenderJobResultV1> = {}): ExportRenderJobResultV1 {
  return {
    ...
    pageSlug: slug("home"),
    size: { w: 80, h: 24 },
    outcome: { manifestIndex: 0, styledFrame: new Uint8Array([1]), textFrame: new TextEncoder().encode("HOME @ 80x24"), layout: new Uint8Array([9]) },
    cacheHit: false,
    ...overrides,
  }
}

function fileMap(files: readonly { readonly relPath: string; readonly bytes: Uint8Array }[]): Map<string, Uint8Array> {
...
describe("assembleExportPackage", () => {
  test("assembles design-prompt.md, runtime-api.json, one page copy, one layout stub, and one snapshot per render", () => {
    const result = assembleExportPackage({ snapshot: SNAPSHOT, renders: [renderResult()], runtimeDeclaration: RUNTIME_DECLARATION })

    expect(result.kind).toBe("assembled")
    if (result.kind !== "assembled") return

    const paths = result.files.map((f) => f.relPath).sort()
    expect(paths).toEqual(["design-prompt.md", "layout/home.json", "pages/home/page.tsx", "runtime-api.json", "snapshots/home/80x24.txt"])
  })
  ...
  test("two sizes for the same page produce two distinct snapshot files, not one overwritten", () => {
    const result = assembleExportPackage({
      snapshot: SNAPSHOT,
      renders: [renderResult(), renderResult({ size: { w: 120, h: 40 }, outcome: { manifestIndex: 0, styledFrame: new Uint8Array(), textFrame: new TextEncoder().encode("HOME @ 120x40"), layout: new Uint8Array() } })],
      runtimeDeclaration: RUNTIME_DECLARATION,
    })
    if (result.kind !== "assembled") throw new Error("expected assembled")
