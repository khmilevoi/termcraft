...
describe("createLeaseStore (injected lock)", () => {
  test("acquires, mints a nonce, and writes advisory diagnostics while holding the lock", async () => {
    const root = makeRoot();
    const lease = await fakeStore({ held: false, releases: 0 }, "nonce-under-test-01").acquire(
      root,
    );
  ...
  test("crashed-owner path re-acquires the existing file and rewrites diagnostics, never deleting it", async () => {
    ...
    fs.writeFileSync(
      lockPath,
      JSON.stringify({ pid: 111, startedAt: "2020-01-01T00:00:00Z", hostname: "gone", nonce: "x" }),
    );
    const before = fs.statSync(lockPath).birthtimeMs;

    const lease = await fakeStore({ held: false, releases: 0 }, "fresh-owner-nonce-1").acquire(
      root,
    );
    expect(lease).not.toBeInstanceOf(Error);
    if (lease instanceof Error) return;
  ...
  test("a held lock with an absent or garbage lock file yields an all-null advisory", async () => {
    ...
    if (heldMissing instanceof LeaseHeldError) {
      expect(heldMissing.advisory).toEqual({
        pid: null,
        startedAt: null,
        hostname: null,
        nonce: null,
    ...
    if (heldGarbage instanceof LeaseHeldError) {
      expect(heldGarbage.advisory).toEqual({
        pid: null,
        startedAt: null,
        hostname: null,
        nonce: null,
...
describe("PID is never ownership proof", () => {
  test("a forged live PID in the file never authorizes a takeover of a held lock", async () => {
    ...
    fs.writeFileSync(
      leaseLockPath(root),
      JSON.stringify({ pid: process.pid, hostname: os.hostname(), nonce: null }),
    );
  ...
  test("an absent/dead PID in the file neither blocks nor is trusted when the lock is free", async () => {
    ...
    fs.writeFileSync(
      leaseLockPath(root),
      JSON.stringify({ pid: 999999, startedAt: null, hostname: "dead", nonce: null }),
    );
    const lease = await fakeStore({ held: false, releases: 0 }, "taken-over-by-lock01").acquire(
      root,
    );
    expect(lease).not.toBeInstanceOf(Error);
    if (lease instanceof Error) return;
    // Acquisition came from the OS lock, and the stale file was rewritten, not deleted.
    expect(JSON.parse(fs.readFileSync(leaseLockPath(root), "utf8")).nonce).toBe(
      "taken-over-by-lock01",
    );
  });
});
