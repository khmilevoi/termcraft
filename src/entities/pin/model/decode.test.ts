...
import { PinDecodeError, decodeCommentsHeader, decodePinEvent, foldPins } from "./decode";
...
describe("decodeCommentsHeader", () => {
  ...
  test("rejects wrong kind, version, id, and slug", () => {
    ...
    expect(
      decodeCommentsHeader({ kind: "pins", formatVersion: 1, projectId: "x", pageSlug: "home" }),
    ).toBeInstanceOf(PinDecodeError);
    expect(
      decodeCommentsHeader({
        kind: "pins",
...
describe("decodePinEvent", () => {
  ...
  test("rejects an unknown kind", () => {
    expect(
      decodePinEvent({ kind: "pin:deleted", recordId: uuidv7(), pinId: uuidv7(), ts: TS }),
    ).toBeInstanceOf(PinDecodeError);
  });
