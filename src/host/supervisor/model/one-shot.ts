...
export async function runOneShotSession(
  ...
  if (spec.mode !== "smoke" && spec.mode !== "export") {
    return new SupervisorError({ code: "TRANSPORT_ERROR", reason: `runOneShotSession requires a smoke/export mode (was "${spec.mode}")` })
  }
  ...
  const helloBytes = encodeClientHello(buildClientHello({ spec, identity, runtimeDeclaration: deps.runtimeDeclaration, offeredLimits }))
  ...
  const helloMessage = await nextInbound(helloDeadline, new SupervisorError({ code: "HANDSHAKE_TIMEOUT", reason: "no host.hello within 3s" }))
  if (helloMessage instanceof Error) return fail(helloMessage)
  if (helloMessage.messageClass !== "control") return fail(new ProtocolError({ code: "MALFORMED_PROTOCOL", reason: "expected a control-class host.hello" }))
  ...
  async function awaitReadyFrame(
    ...
    const timeoutError = new SupervisorError({ code: "MOUNT_TIMEOUT", reason: "no ready + sealed frame within 10s" })
    while (true) {
...
function mapHostError(envelope: ControlEnvelope): ProtocolError | SupervisorError {
  ...
  const reason = typeof rawReason === "string" ? rawReason.slice(0, 200) : "host error during one-shot mount"
