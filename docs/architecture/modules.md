...
1. Eight components, each with a single responsibility:
   ...
   | Component | Owns |
   |------|----------------|
| **UI shell** | Screens; input interpretation (keys, mouse, the composer slash menu); preview compositing of host frames; `/commit-page`, `/commit-infra`, and `/commit-all` command-row status plus the shared commit-confirmation presentation; the action table — the single registry of user actions: hotkey or slash command, availability predicate, dispatched command |
   | **Kernel** | The only decision-maker: commands in, events out, the agent-turn lifecycle (staging assembly → turn → gate → apply), the turn-time locks, the design host's lifecycle; Git history/Restore decisions and scoped-commit planning, confirmation-time `targetChatId` capture and `restoreActionId` binding, pre-write freshness revalidation, in-memory `record-pending` recovery, and project-write mutex coordination through the `GitHistory` and `GitCommitter` ports |
...
8. Git interaction does not share the agent-turn lock wholesale. In v1 the composer can enter local slash-command mode while ordinary message sending is locked; `/commit-page`, `/commit-infra`, and `/commit-all` remain available according to their scope state while other turn-locked command rows stay dimmed. During that turn scoped commit execution contends through the project-write mutex only with short final apply, not network streaming, validation, or Gate. Restore remains locked during the turn; outside agent turns, its ordered canonical-page replacement and captured-target-chat record append use the same mutex. At confirmation the Kernel captures the current chat as `targetChatId`, stores it in the Restore action plan, and creates a unique `restoreActionId` bound to that `targetChatId`, the page, full commit id, and confirmed pre-Restore source hash. Initial execution under the mutex revalidates that hash, Gates the exact snapshot, replaces the canonical source once, then reads and writes the captured target chat file and appends a record containing `restoreActionId`, page, and full commit id. The containing chat file is the record's persisted chat identity; the record itself does not add `targetChatId`. These writes are ordered but not one crash-atomic transaction. If append fails after replacement, the restored page stays active in explicit `record-pending`, which retains `targetChatId` in memory. Append-only **Retry record** checks the captured target chat for `restoreActionId` under the same mutex and either accepts the existing record or appends exactly one there; it never repeats Gate or source replacement and does not revalidate the pre-Restore hash, so ambiguous acknowledgement is idempotent and subsequent page drift does not invalidate the audit fact. Chat creation and switching need not be globally blocked by `record-pending`; changing the UI active chat changes neither the action target nor its exactly-one semantics. A new Restore requires a new confirmation and `restoreActionId`. In v1 `record-pending` is in memory only: after a process crash or restart the source remains on disk, no Retry or automatic audit-record recovery runs, the UI refreshes from disk and Git status, and the missing record may require manual or operator diagnosis. While a historical snapshot is selected, Send, Tweaks, pin creation, pin reopening, and pin-status changes are disabled until the user returns to Current design; existing current pins are only overlaid where their ids resolve.
