...
import { buildExtendedLimitInfo, buildTreeFromSyscalls, createFakeProcessTree, KILL_ON_JOB_CLOSE } from "./job-object"
...
test("[9] adopt() reports a ProcessTreeError when AssignProcessToJobObject fails", () => {
  const tree = buildTreeFromSyscalls(scriptSyscalls({ AssignProcessToJobObject: () => 0 }), fakeHandle())
