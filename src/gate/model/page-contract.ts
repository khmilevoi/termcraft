...
function parseValue(toks: Tok[], i: number): { value: Literal; next: number } | "invalid" {
  ...
  if (t.kind === SK.StringLiteral)
    return terminated(i + 1) ? { value: { kind: "string", s: t.value }, next: i + 1 } : "invalid";
  if (t.kind === SK.NumericLiteral)
    return terminated(i + 1)
      ? { value: { kind: "number", n: Number(t.value) }, next: i + 1 }
      : "invalid";
  if (t.kind === SK.MinusToken && toks[i + 1]?.kind === SK.NumericLiteral) {
    return terminated(i + 2)
      ? { value: { kind: "number", n: -Number(toks[i + 1]!.value) }, next: i + 2 }
      : "invalid";
  }
  if (t.kind === SK.TrueKeyword)
    return terminated(i + 1) ? { value: { kind: "bool", b: true }, next: i + 1 } : "invalid";
  if (t.kind === SK.FalseKeyword)
    return terminated(i + 1) ? { value: { kind: "bool", b: false }, next: i + 1 } : "invalid";
...
export function checkPageContract(
  source: string,
  syntax: SourceSyntax,
): SourceStreamTruncatedError | PageContractResult {
  ...
  const metaIndex = toks.findIndex(
    (t, i) =>
      ...
      t.value === "meta" &&
      i > 0 &&
      BINDING_KEYWORDS.has(toks[i - 1]!.kind),
  );
  let meta: PageMeta | null = null;
  if (metaIndex === -1) {
    err(
      "NON_STATIC_META",
      "no `meta` binding found — a page must `export const meta = definePage({...})`",
      0,
    );
  } else if (
    ...
    err(
      "NON_STATIC_META",
      "`meta` must be a direct `definePage({...})` call with an object-literal argument",
      toks[metaIndex]!.pos,
    );
  } else {
    ...
    if (parsed === "invalid") {
      err(
        "NON_STATIC_META",
        "the `meta` object must contain only literal fields — no spreads, computed keys, variable references, calls, or conditionals",
        pos,
      );
  ...
  const defIdx = toks.findIndex(
    (t, i) => t.kind === SK.ExportKeyword && toks[i + 1]?.kind === SK.DefaultKeyword,
  );
  if (defIdx === -1) {
    ...
    if (after?.value !== "reatomComponent" || toks[defIdx + 3]?.kind !== SK.OpenParenToken) {
      err(
        "NON_REATOM_DEFAULT",
        "the default export must be a `reatomComponent(...)` (the page component)",
        toks[defIdx]!.pos,
      );
    }
  }
...
function validateMetaShape(
  ...
  if (kit?.kind === "number" && Number.isInteger(kit.n) && !SUPPORTED_KIT_API_VERSIONS.has(kit.n)) {
    err(
      "UNSUPPORTED_KIT_API",
      `kitApiVersion ${kit.n} is not supported (this binary accepts ${[...SUPPORTED_KIT_API_VERSIONS].join(", ")})`,
      pos,
    );
    ok = false;
  }
