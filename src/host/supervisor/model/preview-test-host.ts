    ...
    for await (const message of readInbound(carrier)) {
      ...
      if (raw.kind === "set-mode") {
        ...
        return send({
          ...
          messageId: nextId(),
          responseTo: raw.requestId,
          body: { interactionMode: echo },
        });
      }
      if (
        raw.kind === "query-hit" ||
        raw.kind === "query-rect" ||
        raw.kind === "query-describe" ||
        raw.kind === "query-layout"
      ) {
        ...
        const replyBody = options?.queryReply
          ...
          : {
              ok: true,
              frameIdentity: requestBody.frameIdentity,
              result: echoQueryFields(raw.kind, requestBody),
            };
        return send({
          protocolVersion: 1,
          kind: raw.kind,
          ...
          messageId: nextId(),
          responseTo: raw.requestId,
          body: replyBody as ControlEnvelope["body"],
        });
      }
