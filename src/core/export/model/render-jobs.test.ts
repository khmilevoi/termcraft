...
import type { ExportRenderPoolBoundsV1, ExportRenderPort, ExportRenderResultV1, ExportRenderTaskV1, RuntimeDeclarationBundleV1 } from "core/ports"
import type { FailureDtoV1 } from "core/protocol"
import { createFakeExportRenderPort, createFakePageStore, createFakeProjectWriteCoordinator, createFakeRenderCache } from "core/ports/fakes"
...
describe("runExportRendering", () => {
  test("dispatches one render task per (page, ladder size), ordered by (manifestIndex, w, h)", async () => {
    ...
    const results = await runExportRendering({ renderPort, renderCache, rendererVersion: RENDERER_VERSION }, SNAPSHOT)
    ...
    expect(renderPort.calls.map((c) => [c.task.sourceHash, c.task.theme, c.task.size.w, c.task.size.h])).toEqual([
  ...
  test("a cache hit is returned without calling renderOne, and is marked cacheHit", async () => {
    ...
    await renderCache.put({ key, styledFrame: new Uint8Array([9]), textFrame: new Uint8Array([8]), layout: new Uint8Array([7]) })
    ...
    if (hit === undefined || "code" in hit.outcome) throw new Error("expected a successful cached outcome")
  ...
  test("results are ordered by (manifestIndex, w, h) even when the LATER task settles FIRST", async () => {
    ...
    const pageA: ExportPageSnapshotV1 = { ...HOME, pageSlug: slug("a-page"), manifestIndex: 0, minSize: { w: 200, h: 50 } }
    const pageB: ExportPageSnapshotV1 = { ...HOME, pageSlug: slug("b-page"), manifestIndex: 1, minSize: { w: 210, h: 60 } }
    const twoPageSnapshot: ExportSnapshotV1 = { pages: [pageA, pageB], capturedAt: SNAPSHOT.capturedAt }

    const poolBounds: ExportRenderPoolBoundsV1 = { minWorkers: 1, maxWorkers: 1, readyQueueMultiplier: 1 }
    ...
    const pending = runExportRendering({ renderPort: controlledPort, renderCache, rendererVersion: RENDERER_VERSION }, twoPageSnapshot)
    ...
    if (bResolve === undefined || aResolve === undefined) throw new Error("expected both tasks to be pending")

    const bResult: ExportRenderResultV1 = { manifestIndex: 1, styledFrame: new Uint8Array([1]), textFrame: new Uint8Array([1]), layout: new Uint8Array([1]) }
    const aResult: ExportRenderResultV1 = { manifestIndex: 0, styledFrame: new Uint8Array([0]), textFrame: new Uint8Array([0]), layout: new Uint8Array([0]) }
  ...
  test("integration: the permit is acquired and released (snapshot.ts) before the first renderOne call ever fires", async () => {
    ...
    const capturePromise = context.start(() => {
      ...
      return captureExportSnapshot(
        ...
        { pages: [{ pageSlug: HOME.pageSlug, sourcePath: HOME.sourcePath, manifestIndex: HOME.manifestIndex, minSize: HOME.minSize, theme: HOME.theme, kitApiVersion: HOME.kitApiVersion }] },
      )
    })
    ...
    await runExportRendering({ renderPort, renderCache, rendererVersion: RENDERER_VERSION }, snapshotResult.snapshot)
