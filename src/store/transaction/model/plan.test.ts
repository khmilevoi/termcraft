    ...
    operations: [
      {
        ...
        newImage: {
          state: "file",
          sha256: sha256Hex(bytesOf(`header\n${'{"kind":"pin:created"}\n'}`)),
...
describe("computePlanHash", () => {
  test("is deterministic and independent of the operations array's key order", () => {
    ...
    const reordered: TransactionPlan = {
      ...plan,
...
describe("decodePlan", () => {
  ...
  test.each([
    [
      "another transaction's committed marker",
      `${TRANSACTIONS_LOCAL_DIR}/${OTHER_TX_ID}/committed.json`,
    ],
...
describe("validatePlanPayloads", () => {
  ...
  test("rejects a replace operation whose declared newImage.size exceeds its namespace's per-file limit — before intent, with no payload required", () => {
    ...
    const oversized = {
      ...
      newImage: {
        state: "file" as const,
        sha256: op.newImage.state === "file" ? op.newImage.sha256 : "",
        size: 1 * MiB + 1,
      },
    };
    ...
    expect(result instanceof StorageLimitExceededError && result.limit).toBe(
      "project-config per-file",
    );
