...
function defaultCacheRoot(): string {
  ...
  if (localAppData !== undefined && localAppData.length > 0) return path.join(localAppData, "termcraft")
  return os.tmpdir()
}

/** Every `lib.*.d.ts` in the asset dir — including the `lib.d.ts` startup sentinel. */
function listLibNames(assetDir: string): string[] {
  return fs.readdirSync(assetDir).filter((name) => name.startsWith("lib.") && name.endsWith(".d.ts"))
}
 ...
 */
function isCacheComplete(params: { dir: string; exe: string; assetExe: string; libNames: readonly string[] }): boolean {
...
export function materializeCompiler(assets: CompilerAssets): CompilerExtractError | string {
  ...
  return errore.try({
    try: () => {
      ...
      extractCompiler({ dir, exe, assetExe, assetDir: assets.assetDir, libNames, version: assets.version })
      return exe
    },
