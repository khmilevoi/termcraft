...
describe("verifyHostHello", () => {
  ...
  test("rejects an unsupported source kit API with KIT_API_MISMATCH", () => {
    ...
    const result = verifyHostHello(hostHello({ runtimeDeclaration: declV2 }), {
      ...inputs,
      runtimeDeclaration: declV2,
      spec: { ...spec, kitApiVersion: 1 },
      identity: { ...identity, kitApiVersion: 1 },
    });
    expect(result).toBeInstanceOf(ProtocolError);
  ...
  test("rejects child limits larger than offered with MALFORMED_PROTOCOL", () => {
    const tooBig: PublicLimits = {
      ...PROTOCOL_HARD_LIMITS,
      maxFrameCells: PROTOCOL_HARD_LIMITS.maxFrameCells + 1,
    };
