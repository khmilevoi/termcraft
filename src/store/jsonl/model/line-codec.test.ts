...
describe("physical line format", () => {
  test("a line is UTF-8, has no BOM, and ends with exactly one LF", () => {
    ...
    expect(new TextDecoder("utf-8", { fatal: true }).decode(line.subarray(0, -1))).toContain(
      '"kind":"chat"',
    );
  });
...
describe("the 1 MiB physical line bound (storage-identity §11.1)", () => {
  ...
  test("one byte over is rejected, reporting measured vs allowed", () => {
    ...
    expect((rejected as unknown as { measured: number }).measured).toBe(
      JSONL_MAX_PHYSICAL_LINE_BYTES + 1,
    );
    expect((rejected as unknown as { allowed: number }).allowed).toBe(
      JSONL_MAX_PHYSICAL_LINE_BYTES,
    );
  });
...
describe("round trips", () => {
  test("every chat record kind survives encode → decode", () => {
    const records = [
      ...
      {
        kind: "system:cancelled" as const,
        recordId: RECORD_ID,
        turnId: TURN_ID,
        text: "cancelled",
        ts: TS,
  ...
  test("the comments header and pin events survive encode → decode", () => {
    ...
    expect(unwrap(decodeCommentsHeaderLine(unwrap(encodeCommentsHeaderLine(header))))).toEqual(
      header,
    );
