    ...
    for await (const message of readInbound(carrier)) {
      ...
      if (raw.kind === "mount") {
        if (opts?.mountErrorCode !== undefined) {
          send({ protocolVersion: 1, kind: "error", sessionId: id.sessionId, nonce: id.nonce, messageId: nextId(), responseTo: raw.requestId, body: { code: opts.mountErrorCode, reason: "one-shot mount failed" } })
          child.simulateExit({ code: 1 })
          return
        }
        send({ protocolVersion: 1, kind: "ready", sessionId: id.sessionId, nonce: id.nonce, messageId: nextId(), responseTo: raw.requestId, body: { size: { w: spec.size.w, h: spec.size.h }, interactionMode: "static" } })
        if (opts?.skipFrame) return // ready but no frame → the runner times out on capture; child stays alive
        const frame: FrameEnvelope = { protocolVersion: 1, kind: "frame", sessionId: id.sessionId, nonce: id.nonce, sourceHash: spec.sourceHash, frameSeq: "1", width: spec.size.w, height: spec.size.h, rows: Array.from({ length: spec.size.h }, () => []) }
...
describe("runOneShotSession (2D-4, §4/§11.3/§11.4)", () => {
  test("a smoke session seals one frame, the child exits 0, and it returns a nonce-free result", async () => {
    ...
    const result = await runOneShotSession(spec, { spawn: () => child, command: { cmd: ["_host"] }, clock, runtimeDeclaration })
  ...
  test("no host.hello within 3 s is a HANDSHAKE_TIMEOUT", async () => {
    ...
    const clock = createManualClock()
    const promise = runOneShotSession(spec, { spawn: () => child, command: { cmd: ["_host"] }, clock, runtimeDeclaration })
