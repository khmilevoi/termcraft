...
function checkpointFor(records: readonly ChatRecord[], recordCount: number, overrides: Partial<SessionCheckpoint> = {}): SessionCheckpoint {
...
function resume(input: { bytes: Uint8Array; checkpoint: SessionCheckpoint; scopeId?: string; chatId?: string; currentUserRecordId?: string | null }) {
...
describe("computeSessionPrefixHash", () => {
  test("hashes the header line plus exactly recordCount record lines, each including its LF", () => {
    ...
    const computed = unwrap(computeSessionPrefixHash({ chunks: chunksOf(chatBytes(records)), recordCount: 2 }))
  ...
  test("a recordCount of zero hashes the header line alone", () => {
    const computed = unwrap(computeSessionPrefixHash({ chunks: chunksOf(chatBytes([userRecord(1)])), recordCount: 0 }))

    expect(computed.prefixHash).toBe(sha256(headerBytes()))
  })

  test("a prefix longer than the file is an error, not a short hash", () => {
    const computed = computeSessionPrefixHash({ chunks: chunksOf(chatBytes([userRecord(1)])), recordCount: 4 })
  ...
  test("an unterminated final line is not a record", () => {
    ...
    const bytes = concat([headerBytes(), recordBytes(userRecord(1)), partial.subarray(0, partial.byteLength - 1)])

    expect(computeSessionPrefixHash({ chunks: chunksOf(bytes), recordCount: 2 })).toBeInstanceOf(SessionPrefixError)
  })

  test("a negative or fractional recordCount is rejected", () => {
    const bytes = chatBytes([userRecord(1)])

    expect(computeSessionPrefixHash({ chunks: chunksOf(bytes), recordCount: -1 })).toBeInstanceOf(SessionPrefixError)
    expect(computeSessionPrefixHash({ chunks: chunksOf(bytes), recordCount: 1.5 })).toBeInstanceOf(SessionPrefixError)
  })
})
...
describe("evaluateSessionResume", () => {
  ...
  test("a shorter file is a mismatch", () => {
    ...
    const decision = resume({ bytes: chatBytes(records.slice(0, 2)), checkpoint: checkpointFor(records, 3) })
  ...
  test("a header naming a different chat is an identity mismatch", () => {
    ...
    const decision = resume({ bytes: chatBytes(records, OTHER_CHAT_ID), checkpoint: checkpointFor(records, 1) })
  ...
  test("a checkpoint for another chat is an identity mismatch", () => {
    ...
    const decision = resume({ bytes: chatBytes(records), checkpoint: checkpointFor(records, 1, { chatId: OTHER_CHAT_ID }) })
  ...
  test("a different session scope is a mismatch", () => {
    ...
    const decision = resume({ bytes: chatBytes(records), checkpoint: checkpointFor(records, 1), scopeId: "claude-code:opus:acct-9:ws-1" })
  ...
  test("a corrupt line inside the log is a mismatch even when the checkpointed prefix still matches", () => {
    const records = [userRecord(1), userRecord(2)]
    const bytes = concat([headerBytes(), recordBytes(records[0]!), new TextEncoder().encode("{not json\n"), recordBytes(records[1]!)])
  ...
  test("a mismatch carries the bounded fresh seed built from the valid prefix", () => {
    ...
    const decision = resume({ bytes: chatBytes(records), checkpoint: checkpointFor(records, 3, { prefixHash: "0".repeat(64) }) })

    expect(decision.kind).toBe("fresh")
    if (decision.kind !== "fresh") return
    expect(decision.seed.records.map((record) => record.recordId)).toEqual([recordId(1), recordId(2), recordId(3)])
  })
  ...
  test("the current user record and everything after it stay out of the seed", () => {
    ...
    expect(decision.kind).toBe("fresh")
    if (decision.kind !== "fresh") return
    expect(decision.seed.records.map((record) => record.recordId)).toEqual([recordId(1), recordId(2)])
  })
})
...
describe("selectSeedRecords", () => {
  test("keeps every record in chronological order when both bounds allow", () => {
    ...
    expect(seed.records.map((record) => record.recordId)).toEqual([recordId(1), recordId(2), recordId(3)])
  })
  ...
  test("drops whole records oldest-first to stay within the 64 KiB text bound", () => {
    const records = Array.from({ length: 20 }, (_unused, at) => userRecord(at + 1, "x".repeat(10_000)))
  ...
  test("the bound is measured in UTF-8 bytes, not code units, and the exact bound fits", () => {
    ...
    const records = [userRecord(1, "é".repeat(16_384)), userRecord(2, "é".repeat(16_384)), userRecord(3, "é".repeat(16_384))]
  ...
  test("a newest record that alone exceeds the byte bound yields an empty seed", () => {
    const records = [userRecord(1, "small"), userRecord(2, "x".repeat(SESSION_SEED_MAX_TEXT_BYTES + 1))]
