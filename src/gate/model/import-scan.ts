  ...
  for (let i = 0; i < toks.length; i += 1) {
    ...
    if (t.kind === SK.ImportKeyword) {
      ...
      if (next?.kind === SK.OpenParenToken) {
        ...
        errors.push({ code: "DYNAMIC_IMPORT", specifier: spec?.value ?? "", message: `dynamic import("${spec?.value ?? ""}") is not allowed — a page loads no runtime-selected code`, line: where.line, column: where.column })
        continue
      }
      ...
      if (specifier !== null && specifier !== RUNTIME_ROOT) {
        const where = at(t.pos)
        errors.push({ code: "FORBIDDEN_IMPORT", specifier, message: `import of "${specifier}" is not allowed — a page may only import "${RUNTIME_ROOT}"`, line: where.line, column: where.column })
      }
      continue
    }
    ...
    if (t.kind === SK.ExportKeyword) {
      ...
      if (next?.kind === SK.TypeKeyword) {
        ...
        if (afterType !== undefined && afterType.kind !== SK.OpenBraceToken && afterType.kind !== SK.AsteriskToken) continue
      }
      let specifier: string | null = null
      for (let j = i + 1; j < toks.length; j += 1) {
        const tj = toks[j]!
      ...
      if (specifier !== null) {
        const where = at(t.pos)
        errors.push({ code: "REEXPORT", specifier, message: `re-export from "${specifier}" is not allowed — a page exports no module edge`, line: where.line, column: where.column })
      }
      continue
    }
    ...
    if (t.kind === SK.RequireKeyword && next?.kind === SK.OpenParenToken) {
      ...
      if (spec !== null) {
        const where = at(t.pos)
        errors.push({ code: "REQUIRE_CALL", specifier: spec.value, message: `require("${spec.value}") is not allowed — a page uses no CommonJS load`, line: where.line, column: where.column })
      }
      continue
    }
  }
