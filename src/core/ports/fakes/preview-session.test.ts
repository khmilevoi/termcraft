...
const FAILURE: FailureDtoV1 = { code: "HOST_PROTOCOL_FAILED", retryable: true, safeMessage: "host disconnected", details: {} }

function frame(frameSeq: string): PreviewFrameV1 {
  return { sessionId: "sess-1", sourceHash: identity.sourceHash, frameSeq, width: 80, height: 24, rows: [] }
}
...
describe("createFakePreviewSession", () => {
  ...
  test("queueQueryResult() scripts a resolved anchor for the next query(), one shot", async () => {
    const session = createFakePreviewSession(identity)
    session.queueQueryResult({ result: {}, resolvedAnchor: { pageSlug: "home", elementId: "btn-1", fx: 0.5, fy: 0.5 } })
    const first = await session.query("token-1", { kind: "hit", x: 1, y: 1 })
    if ("code" in first) throw new Error("unexpected failure")
    expect(first.resolvedAnchor).toEqual({ pageSlug: "home", elementId: "btn-1", fx: 0.5, fy: 0.5 })
