...
export type SetPinStatusResultV1 = { readonly kind: "not-found" } | { readonly kind: "updated"; readonly payload: PinsChangedPayloadV1 }
...
export async function setPinStatus(
  ...
  if (before instanceof Error) {
    return { code: "PERSISTENCE_FAILED", retryable: false, safeMessage: before.message, details: {} }
  }
  ...
  if (after instanceof Error) {
    return { code: "PERSISTENCE_FAILED", retryable: false, safeMessage: after.message, details: {} }
  }
  ...
  if (updated === undefined) {
    ...
    console.error(`core/pins: pinId ${input.pinId} vanished from the fold immediately after its own status append`)
    return { code: "PERSISTENCE_FAILED", retryable: false, safeMessage: "pin projection lost its own pin", details: {} }
  }
