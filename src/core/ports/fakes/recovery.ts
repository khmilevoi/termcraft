...
import type { OrphanTurnOutcomeV1, RecoveryOutcomeV1, RecoveryService, TransactionClassificationV1 } from "../recovery"
...
const DEFAULT_RECOVER_OUTCOME: RecoveryOutcomeV1 = { ok: true, recovered: 0, discarded: 0, alreadyComplete: 0 }
...
export function createFakeRecoveryService(options?: {
  ...
  async function classify(transactionId: string): Promise<FailureDtoV1 | TransactionClassificationV1> {
  ...
  return { recover, classify, hasIntentRecord, scanOrphanTurns, calls, failNext, scriptRecoverOutcome, setIntentRecord }
}
