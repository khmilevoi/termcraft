 ...
 */
const DETERMINISTIC_SUPERVISOR_CODES = new Set<string>(["PROTOCOL_FLOOD", "STDERR_FLOOD", "CONTROL_BACKPRESSURE"])
 ...
 */
export function classifyFailure(error: ProtocolError | SupervisorError): "budgeted" | "deterministic" {
...
export function createRestartPolicy(opts?: {
  ...
  function recordFailure(key: string, error: ProtocolError | SupervisorError, now: number): RestartAction {
    const state = stateOf(key)
    if (state.open) return { action: "open", attempts: state.failures.length, reason: "circuit already open" }
    ...
    if (attempt > maxRestarts) {
      state.open = true
      return { action: "open", attempts: attempt, reason: `restart budget exhausted (${maxRestarts} in ${windowMs}ms)` }
    }
