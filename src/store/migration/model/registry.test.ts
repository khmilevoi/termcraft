...
import { DataFormatTooNewError, MIGRATION_CHAIN, NoMigrationPathError, checkFormatCounter, createMigrationRegistry, findMigrationSteps, migrationRegistry, readFormatCounter } from "./registry"
...
describe("checkFormatCounter", () => {
  test("found <= supported passes", () => {
    expect(checkFormatCounter({ file: "project.toml", field: "format_version", found: 1, supported: 1 })).toBeNull()
    expect(checkFormatCounter({ file: "project.toml", field: "format_version", found: 0, supported: 1 })).toBeNull()
  })

  test("found > supported is a hard error naming the file", () => {
    const result = checkFormatCounter({ file: "project.toml", field: "format_version", found: 2, supported: 1 })
    expect(result).toBeInstanceOf(DataFormatTooNewError)
    if (!(result instanceof DataFormatTooNewError)) throw new Error("expected DataFormatTooNewError")
...
describe("createMigrationRegistry", () => {
  ...
  test("wires findSteps and checkNotTooNew over an injected chain", () => {
    ...
    expect(registry.checkNotTooNew({ file: "widget.json", field: "schemaVersion", found: 3, supported: 2 })).toBeInstanceOf(DataFormatTooNewError)
    expect(registry.checkNotTooNew({ file: "widget.json", field: "schemaVersion", found: 2, supported: 2 })).toBeNull()
  })
})
