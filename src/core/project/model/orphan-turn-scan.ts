 ...
 */
export function classifyOrphanTurns(
  outcomes: readonly OrphanTurnOutcomeV1[],
): readonly OrphanTurnDecisionV1[] {
...
function classifyOne(
  outcome: OrphanTurnOutcomeV1,
  group: readonly OrphanTurnOutcomeV1[],
  ...
  if (outcome.terminalized)
    return { kind: "resolved", chatId: outcome.chatId, turnId: outcome.turnId };
  return {
    kind: "chat-corrupt",
    chatId: outcome.chatId,
    turnId: outcome.turnId,
    reason: "terminal-without-user",
  };
}

/** Runs the port's own scan, then applies {@link classifyOrphanTurns} to its report. */
export async function scanOrphanTurns(
  deps: OrphanTurnScanDeps,
): Promise<FailureDtoV1 | readonly OrphanTurnDecisionV1[]> {
