...
describe("foldGateDiagnosticsIntoPrompt — the diagnostics freshness barrier", () => {
  test("nextAttempt === rejectedAttempt + 1 is fresh and folds", () => {
    const result = foldGateDiagnosticsIntoPrompt(
      foldInput({
        rejectedAttempt: 1 as TurnAttempt,
        nextAttempt: 2 as TurnAttempt,
        diagnostics: diagnostics({ errors: [{ kind: "type", code: "TS2322", message: "boom", file: null, line: null, column: null }] }),
      }),
    )
  ...
  test("nextAttempt === rejectedAttempt (no advance) is stale", () => {
    const result = foldGateDiagnosticsIntoPrompt(foldInput({ rejectedAttempt: 2 as TurnAttempt, nextAttempt: 2 as TurnAttempt }))
    expect(result).toBeInstanceOf(StaleGateDiagnosticsError)
  })

  test("nextAttempt skipping ahead (rejectedAttempt + 2) is stale", () => {
    const result = foldGateDiagnosticsIntoPrompt(foldInput({ rejectedAttempt: 1 as TurnAttempt, nextAttempt: 3 as TurnAttempt }))
    expect(result).toBeInstanceOf(StaleGateDiagnosticsError)
  })

  test("nextAttempt going backward is stale", () => {
    const result = foldGateDiagnosticsIntoPrompt(foldInput({ rejectedAttempt: 3 as TurnAttempt, nextAttempt: 2 as TurnAttempt }))
    expect(result).toBeInstanceOf(StaleGateDiagnosticsError)
  })

  test("stale error carries both attempt numbers", () => {
    const result = foldGateDiagnosticsIntoPrompt(foldInput({ rejectedAttempt: 1 as TurnAttempt, nextAttempt: 4 as TurnAttempt }))
    if (!(result instanceof StaleGateDiagnosticsError)) throw new Error("expected a StaleGateDiagnosticsError")
...
describe("foldGateDiagnosticsIntoPrompt — folding errors and determinism warnings", () => {
  test("errors are rendered under a header, one line per error", () => {
    const result = foldGateDiagnosticsIntoPrompt(
      foldInput({
        diagnostics: diagnostics({
          errors: [
            { kind: "type", code: "TS2322", message: "Type 'string' is not assignable to 'number'.", file: "pages/home.tsx", line: 12, column: 4 },
            { kind: "import", code: "IMPORT_NOT_ALLOWED", message: "disallowed import", file: null, line: null, column: null },
          ],
        }),
      }),
    )
  ...
  test("determinism warnings (unguarded-timer, unguarded-randomness) ARE folded in", () => {
    const result = foldGateDiagnosticsIntoPrompt(
      foldInput({
        diagnostics: diagnostics({
          errors: [{ kind: "type", code: "TS2322", message: "boom", file: null, line: null, column: null }],
          warnings: [
            { kind: "unguarded-timer", message: "setTimeout without a seeded clock", line: 5, column: null },
            { kind: "unguarded-randomness", message: "Math.random without a seed", line: null, column: null },
          ],
        }),
      }),
    )
      ...
      foldInput({
        diagnostics: diagnostics({
          ...
          warnings: [
            ...
            { kind: "unlisted-navigation", message: "navigation target not declared", line: null, column: null },
          ],
        }),
      }),
    )
