...
- `src/gate/model/import-scan.ts` — the static `@termcraft/runtime`-only import allowlist scan (tokenizer-based, not regex), plus a FATAL dynamic-code rejection (design §5.8) for `eval`/`Function` — a direct call, a bare `eval` reference reached through any indirection, and the computed-string evasions (`globalThis["eval"]`) — using `jsx.ts`'s JSX-text token set so a page's own display copy containing the word "eval"/"Function" is never mistaken for a reference
- `src/gate/model/jsx.ts` — the JSX open-tag recognizer (`scanOpenTag`/`readHyphenatedName`) shared by `lints.ts` and `import-scan.ts`, plus `computeJsxTextTokenIndices`: the token indices that are JSX children text rather than code, built by requiring a genuine matching closing tag so an uncalled generic type-argument list (`Array<Foo>`, a `scanOpenTag`-accepted residual gap) can never mask a real dynamic-code reference as bogus "text"
