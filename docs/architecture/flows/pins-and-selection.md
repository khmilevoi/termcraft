...
7. Historical overlays: a pin is drawn on a historical snapshot only when its element id resolves in that historical render, using the stored fractional position in the historical element rectangle. Browsing history never restores pin data and never changes pin records or their open/resolved statuses; Restore also leaves them untouched.
