...
function parseManifest(text: string): { error: GateError } | { pages: unknown; active: unknown } {
  ...
  if (parsed instanceof Error) return { error: manifestError("MANIFEST_PARSE", `pages.json is not valid JSON: ${parsed.message}`) }
  if (typeof parsed !== "object" || parsed === null || Array.isArray(parsed)) {
    return { error: manifestError("MANIFEST_SHAPE", "pages.json must be a JSON object with a `pages` array") }
  }
...
export function checkManifestSlice(input: ManifestScanInput): ManifestScanResult {
  ...
  if (!Array.isArray(parsed.pages)) {
    errors.push(manifestError("MANIFEST_SHAPE", "pages.json `pages` must be an array of slug strings"))
    return { errors, slice: null }
  }
  if (parsed.active !== undefined && typeof parsed.active !== "string") {
    errors.push(manifestError("MANIFEST_SHAPE", "pages.json `active`, when present, must be a slug string"))
  }
  ...
  for (const entry of parsed.pages) {
    ...
    if (seen.has(slug)) {
      errors.push(manifestError("MANIFEST_DUPLICATE_SLUG", `pages.json lists "${slug}" more than once`))
      continue
    }
    seen.add(slug)
    slugs.push(slug)
  ...
  for (const slug of slugs) {
    if (!present.has(slug)) errors.push(manifestError("MANIFEST_UNKNOWN_PAGE", `pages.json lists "${slug}" but no such page file is staged`))
  }
  for (const slug of input.presentSlugs) {
    if (!seen.has(slug)) errors.push(manifestError("MANIFEST_MISSING_PAGE", `staged page "${slug}" is absent from pages.json`))
  }
  ...
  if (typeof parsed.active === "string") {
    ...
    if (parsedActive instanceof Error) {
      errors.push(manifestError("MANIFEST_INVALID_SLUG", `pages.json \`active\`: ${parsedActive.message}`))
    } else if (!seen.has(parsedActive)) {
      errors.push(manifestError("MANIFEST_ACTIVE_NOT_LISTED", `pages.json \`active\` "${parsedActive}" is not in the pages list`))
    } else {
      active = parsedActive
    }
  }
