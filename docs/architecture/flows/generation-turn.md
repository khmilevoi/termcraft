...
2. Staging: machine-local scratch at a stable per-project path, cleared and repopulated at the start of every turn with `pages/<slug>.tsx` (the head of every listed page, flat by slug), `pages.json` (the manifest slice: ordered slugs and the active page), and the kit reference (docs plus type declarations). The path is stable across turns and restarts so that resumed agent sessions' file references from earlier turns stay valid — and the Codex sandbox's writable root stays a configuration constant. The prompt carries the designer role, the design-code rules, the project manifest (page list with cached titles), the outstanding warnings (gate and host-reported), selection, pins, answer-style guidance (final message short, light markdown), and the user message. There is no read protocol and no prefetch policy: the agent reads whatever staging files it wants with its own tools.
...
8. Failure branch: still invalid after the initial attempt plus 3 retries — an honest error is written to chat as a system record, staging is cleared, and head versions are left untouched.
9. Failure branch: cancellation, whether by `Esc` or 120 seconds of stream silence — the SDK run is aborted, a system record is written to chat, staging is cleared, and project state stays intact.
