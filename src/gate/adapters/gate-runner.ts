...
import { lintDeterminism, lintModuleScopeTokens, lintSilencingAny } from "../model/lints";
 ...
 * warnings for ONE code file's text, stamped with `file: relPath` — the SAME three lints
 * `gate/model/gate.ts`'s `runGate` runs per page (`lintDeterminism`, `lintSilencingAny`,
 * `lintModuleScopeTokens`), called here over a file this pass read directly rather than only
 * over a manifest entry's own source. See {@link lintWholeTreeDeterminism}'s doc for why this
 * scans every code file, not only entries.
 *
 * TOLERANT OF AN UNSCANNABLE SOURCE, deliberately, TWO WAYS. All three lints share `tokenize`
