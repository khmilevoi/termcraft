...
describe("projectPinEvents", () => {
  ...
  test("the LAST status wins across several status events, file order authoritative", () => {
    const result = projectPinEvents(PAGE, [
      ...
      status({
        recordId: "0192f6f0-0000-7000-8000-000000000004",
        status: "open",
        actionId: undefined,
        turnId: "0192f6f0-0000-7000-8000-000000000005",
  ...
  test("returns pins in creation order across multiple pins", () => {
    ...
    if (result instanceof Error) throw result;
    expect(result.map((p) => p.pinId)).toEqual([
      "0192f6f0-0000-7000-8000-0000000000b2",
      "0192f6f0-0000-7000-8000-0000000000a1",
    ]);
  });
  ...
  test("a duplicate pin:created for the same pinId is corruption", () => {
    const result = projectPinEvents(PAGE, [
      created(),
      created({ recordId: "0192f6f0-0000-7000-8000-000000000099" }),
    ]);
