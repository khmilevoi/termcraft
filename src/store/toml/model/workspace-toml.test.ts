...
describe("decodeWorkspaceLocalState", () => {
  ...
  test("round-trips the defaults", () => {
    expect(decodeWorkspaceLocalState(encodeWorkspaceLocalState(defaultWorkspaceLocalState()))).toEqual(defaultWorkspaceLocalState())
  })

  test("round-trips the preset size mode", () => {
    const preset: WorkspaceLocalState = { ...populated, previewSizeMode: "preset", previewSizePreset: "120x40", previewCustomWidth: null, previewCustomHeight: null }
  ...
  describe("resource-limit overrides are validated IN RANGE (§6.1, projections §16.1)", () => {
    const cases: ReadonlyArray<{
      ...
      { key: "diagnosticsQuotaBytes", tomlKey: "diagnostics_quota_bytes", min: 32 * 1024 * 1024, max: 512 * 1024 * 1024 },
      { key: "renderCacheQuotaBytes", tomlKey: "render_cache_quota_bytes", min: 64 * 1024 * 1024, max: 4 * 1024 * 1024 * 1024 },
      { key: "exportWorkers", tomlKey: "export_workers", min: 1, max: 8 },
      { key: "previewFps", tomlKey: "preview_fps", min: 1, max: 60 },
      { key: "operationsLogSegmentBytes", tomlKey: "operations_log_segment_bytes", min: 1024 * 1024, max: 20 * 1024 * 1024 },
      { key: "operationsLogRetentionSegments", tomlKey: "operations_log_retention_segments", min: 2, max: 10 },
      { key: "silenceTimeoutSeconds", tomlKey: "silence_timeout_seconds", min: 30, max: 600 },
      { key: "absoluteTurnDeadlineMinutes", tomlKey: "absolute_turn_deadline_minutes", min: 10, max: 60 },
    ]

    for (const { key, tomlKey, min, max } of cases) {
      test(`${tomlKey}: both bounds are accepted, one step outside either bound is invalid`, () => {
        const render = (value: number) => `format_version = 1\n[resource_limits]\n${tomlKey} = ${value}\n`
        ...
        expect(decodeWorkspaceLocalState(render(min - 1))).toBeInstanceOf(WorkspaceStateCorruptError)
        expect(decodeWorkspaceLocalState(render(max + 1))).toBeInstanceOf(WorkspaceStateCorruptError)
      })
    }
  ...
  test("rejects a malformed session checkpoint", () => {
    ...
    expect(decodeWorkspaceLocalState(base + good.replace(PREFIX_HASH, "abc"))).toBeInstanceOf(WorkspaceStateCorruptError)
    expect(decodeWorkspaceLocalState(base + good.replace("record_count = 3", "record_count = -1"))).toBeInstanceOf(
      WorkspaceStateCorruptError,
    )
    expect(decodeWorkspaceLocalState(base + good.replace(`"0190fc4a-8b5c-7d3e-8a91-6f2e4c7b5d20"`, `"nope"`))).toBeInstanceOf(
      WorkspaceStateCorruptError,
    )
  })
})
...
describe("the portable/local split is airtight (§16.1)", () => {
  ...
  test("corrupt local state can never be copied into portable state — the decoders share no field", () => {
    const load = loadWorkspaceLocalState({ text: `format_version = 1\nproject_id = "0190fc4a-8b5c-7d3e-8a91-6f2e4c7b5d10"\n` })
