...
4. After trust, termcraft loads each page's canonical `.termcraft/pages/<slug>/page.tsx`
   ...
   (import-allowlist, page-contract, lints) run this way today; its optional type-check
   stage stays unwired — there is no production `runtimeDts` source yet (phase 8). The
...
9. Failure branch: if the agent CLI is missing or not logged in, the background health check — running since startup — surfaces the problem in the status bar; on Home's error state, `r` re-runs the check in place without restarting termcraft. The check itself is implemented. It runs a minimal query and reads the reply until something classifies it: the CLI announcing itself means installed and ready, an authentication signal means not logged in, and a failure to start the process at all means not installed. It is bounded by a deadline so a CLI that connects and then says nothing cannot hang startup, and it never resolves the ambiguous cases as ready — an inconclusive probe reports a problem rather than letting a paid turn start against a broken backend. It never throws, so a launch sequence cannot be taken down by it.
   ...
   - *Surfacing:* the Home health line and the `r` re-check are now real UI (`home-recheck` re-runs the injected probe via `refreshHomeHealth`), but the composition root does not yet inject the real background probe — the UI reads a placeholder "agent ready" reading today, so the live CLI check does not gate or feed the launch UI yet.
...
- `src/entrypoint/model/create-shell.ts` — the composition root: `openOrCreateProject` opens (or, for a fresh directory, creates) the project on disk BEFORE the Kernel exists, wires the 17 store/gate/host/agent adapters into `createKernel`, adapts the composed `Kernel` to `ui`'s `KernelPort`, and closes in reverse-acquisition order (Kernel → host supervisor → project lease) so a teardown rejection can never abandon the lease. `FrameAcknowledgeNotWiredError` records a still-open gap: `acknowledgeDisplay` no-ops because the Kernel exposes no frame-token surface (blocker B1), leaving geometry queries / hover-pin unusable.
...
- `src/gate/model/gate.ts` — `runGate`: the static import-allowlist, page-contract, and all five warning-lint checks that always run over a candidate source, plus the optional type-check/manifest/smoke-render stages a caller can inject; the `dropped-id` and `unlisted-navigation` lints each take an extra optional `GateInput` field (`referencedIds`, `listedSlugs`) and skip silently when the caller omits it. The composition root now wires this Gate adapter into the Kernel, so the source-only stages run over every canonical source at open; the optional injected type-check stage stays unwired — no production `runtimeDts` source exists yet (phase 8).
...
- `src/ui/app/model/deps.ts` — the startup agent-health probe injection point (`agentHealthProbe`, defaulting to a placeholder reading — the real probe is not injected in production yet) plus the Kernel subscription and preview-frame consumer the mounted app activates.
