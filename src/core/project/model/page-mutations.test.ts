...
import { type PageRemovePlanLedger, createPageRemovePlanLedger } from "./page-remove-plan";
...
describe("renameTitle", () => {
  test("rewrites the title through the page store and invalidates any active remove plan", async () => {
    const { call, readCurrent, pageStore } = context.start(() => {
      ...
      return {
        call: renameTitle({ pageStore, planLedger }, slug("home"), "New title"),
        readCurrent: readerFor(planLedger),
        pageStore,
      };
    });
...
describe("reorder", () => {
  test("replaces the page order and invalidates any active remove plan", async () => {
    const { call, readCurrent, pageStore } = context.start(() => {
      ...
      return {
        call: reorder({ pageStore, planLedger }, [slug("about"), slug("home")]),
        readCurrent: readerFor(planLedger),
        pageStore,
      };
    });
...
function setupConfirmScenario() {
  ...
  const projectStore = createFakeProjectStore({
    root: "C:/proj",
    workspaceState: { activePageSlug: slug("about") },
  });
  ...
  test("acquires the mutex BEFORE revalidating, then removes the page and releases the mutex", async () => {
    const { call, readCurrent, pageStore, mutex, order } = context.start(() => {
      ...
      const call = confirmPageRemove(
        {
          pageStore: tracedPageStore,
          projectStore: tracedProjectStore,
          planLedger,
          mutex: tracedMutex,
        },
