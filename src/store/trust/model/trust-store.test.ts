...
import { TrustLedgerError, TrustSubjectError, createTrustStore, nodeTrustFsDeps, trustGrantPath } from "./trust-store"
 ...
 */
function memoryFs(seed: { readonly links?: Record<string, string>; readonly identities?: Record<string, string> } = {}) {
...
describe("trustGrantPath", () => {
  test("keys the machine-local ledger by the 64-hex trust key", () => {
    ...
    expect(trustGrantPath(USER_STATE_ROOT, key)).toBe(path.join(USER_STATE_ROOT, "trust", `${key}.json`))
  })
})
...
describe("buildSubject", () => {
  ...
  test("canonicalizes the supplied Git common dir and repo-relative path", async () => {
    const memory = memoryFs({ identities: { "C:\\work\\termcraft": "windows:1a2b3c4d:0011223344" } })
    const subject = await storeOver(memory).buildSubject("C:\\work\\termcraft", PROJECT_ID, gitIdentity)
    if (subject instanceof Error) throw subject
  ...
  test("two path aliases resolving to the same canonical object produce ONE subject", async () => {
    ...
    const viaTrailingSeparator = await store.buildSubject("C:\\work\\termcraft\\", PROJECT_ID, null)
  ...
  test("a path move produces a new subject", async () => {
    const memory = memoryFs({
      identities: { "C:\\work\\termcraft": "windows:1a2b3c4d:0011223344", "C:\\moved\\termcraft": "windows:1a2b3c4d:0011223344" },
    })
    const store = storeOver(memory)
  ...
  test("a HEAD, branch, or commit change keeps the SAME subject", async () => {
    ...
    const store = storeOver(memory)
    const before = await store.buildSubject("C:\\work\\termcraft", PROJECT_ID, gitIdentity)
    const afterCheckout = await store.buildSubject("C:\\work\\termcraft", PROJECT_ID, { ...gitIdentity })
    if (before instanceof Error) throw before
...
describe("isGranted / grant", () => {
  ...
  test("a tampered grant record does not grant, because its fields no longer derive its key", async () => {
    ...
    const grantPath = trustGrantPath(USER_STATE_ROOT, subject.key)
    const record = JSON.parse(new TextDecoder().decode(memory.files.get(grantPath) as Uint8Array)) as Record<string, unknown>
  ...
  test("a schema-invalid grant record does not grant", async () => {
    const { memory, store, subject } = await grantedStore()
    await store.grant(subject)
    memory.poison(trustGrantPath(USER_STATE_ROOT, subject.key), JSON.stringify({ schemaVersion: 2, key: subject.key }))
...
describe("nodeTrustFsDeps", () => {
  test("grants and re-reads a real project root through the real filesystem", async () => {
    ...
    const store = createTrustStore({ userStateRoot: stateRoot, clock: frozenClock, fs: nodeTrustFsDeps })
