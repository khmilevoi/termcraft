...
import { createFakeAgentBackend, type FakeAgentBackend } from "core/ports/fakes"
import type { AgentTask } from "core/ports"
import type { Clock } from "infrastructure/clock"

import { STREAM_SILENCE_MS, createTurnDeadlines, type TurnDeadlines } from "./deadlines"
import { createTurnFence, type TurnFence } from "./fence"
import {
  startTurnAttempt,
...
describe("startTurnAttempt — workspace-ready -> running -> stopping", () => {
  test("illegal from a non-workspace-ready phase: returns illegal, mints no lease, starts nothing", async () => {
    await context.start(async () => {
      ...
      const result = startTurnAttempt(h.deps, { turnId: TURN_ID, fence: h.fence, task: baseTask() })
  ...
  test("a valid fenced event notes deadlines.noteEvent and emits turn.progress", async () => {
    await context.start(async () => {
      ...
      expect(h.published).toEqual([
        ...
        {
          kind: "turn.progress",
          payload: { turnId: TURN_ID, attempt: 1, content: { kind: "reasoning", text: "thinking" } },
          correlation: { turnId: TURN_ID },
        },
      ])

      h.agentBackend.completeRun(lease, { kind: "completed", finalText: "done", usage: null, sessionId: "s1" })
  ...
  test("an event carrying a stale fence is dropped: no deadlines.noteEvent, no turn.progress", async () => {
    await context.start(async () => {
      ...
      h.agentBackend.completeRun(staleLease, { kind: "completed", finalText: "done", usage: null, sessionId: "s1" })
      await wrap(handle.outcome)
    })
  })
  ...
  test("backend-error maps to a failed outcome and enters stopping", async () => {
    await context.start(async () => {
      ...
      h.agentBackend.completeRun(lease, { kind: "backend-error", message: "boom", sessionId: "s1" })
  ...
  test("a retry mints attempt 2 with a fresh nonce, keeping the same turnId", async () => {
    await context.start(async () => {
      ...
      h.agentBackend.completeRun(firstLease, { kind: "completed", finalText: "done", usage: null, sessionId: "s1" })
      ...
      h.agentBackend.completeRun(secondLease, { kind: "completed", finalText: "done2", usage: null, sessionId: "s2" })
