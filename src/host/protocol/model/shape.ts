...
export const decimalUint64Schema = z.custom<string>((value) => {
...
export function sortedUniqueNumberArraySchema<T extends z.ZodType<number>>(itemSchema: T) {
  return z.array(itemSchema).refine(isSortedUniqueNumbers, { error: "must be sorted and duplicate-free" })
}
...
export function sortedUniqueStringArraySchema<T extends z.ZodType<string>>(itemSchema: T) {
  return z.array(itemSchema).refine(isSortedUniqueStrings, { error: "must be sorted and duplicate-free" })
}
