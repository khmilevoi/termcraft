...
import type { ChatReader, GitIdentityV1, PageReader, PinReader, ProjectStore, RecoveryService, TrustGate, TrustSubjectV1 } from "core/ports"
...
import { routeProjectRecovery, type RecoveryRoutingMachines } from "./recovery-routing"
...
export interface OpenSequenceDeps {
  ...
  readonly findIntendedRecoveryDomain: () => Promise<FailureDtoV1 | IntendedRecoveryDomainV1 | null>
...
function blocked(deps: OpenSequenceDeps, step: OpenSequenceStepV1, failure: FailureDtoV1): OpenSequenceOutcomeV1 {
...
async function validateProjectContents(deps: OpenSequenceDeps, store: ProjectStore): Promise<FailureDtoV1 | undefined> {
...
async function resolveTrust(deps: OpenSequenceDeps, store: ProjectStore): Promise<FailureDtoV1 | TrustDecisionV1> {
  if (deps.mode === "create") {
    const subject = await grantImplicitTrust({ trustGate: deps.trustGate, root: store.root, projectId: deps.projectId, git: deps.git })
    if ("code" in subject) return subject
    return "trusted"
  }

  const status = await buildTrustStatus({ trustGate: deps.trustGate, root: store.root, projectId: deps.projectId, git: deps.git })
...
export async function runOpenSequence(deps: OpenSequenceDeps): Promise<OpenSequenceOutcomeV1> {
  const startOutcome = deps.machines.project.apply(deps.mode === "create" ? "beginCreate" : "beginOpen")
  ...
  if (intendedDomain !== null && intendedDomain !== "restore" && intendedDomain !== "export" && intendedDomain !== "migration") {
  ...
  if (contentValidation !== undefined) return blocked(deps, "content-validation", contentValidation)
