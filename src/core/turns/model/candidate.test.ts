...
import { createFakeStagingService, type FakeStagingService } from "core/ports/fakes"
...
import { freezeTurnCandidate, type FreezeTurnCandidateDeps } from "./candidate"
...
function queueCandidate(staging: FakeStagingService, files: TurnWorkspaceV1["files"], totalBytes = 0): void {
...
describe("freezeTurnCandidate — snapshotting -> validating", () => {
  ...
  test("the happy path calls staging.snapshotToCandidate with the exact workspace, then candidateCaptured", async () => {
    await context.start(async () => {
      ...
      const result = await wrap(freezeTurnCandidate(h.deps, { workspace: h.workspace }))
      if (result.kind !== "captured") throw new Error(`expected captured, got ${JSON.stringify(result)}`)
  ...
  test("diff: unchanged page (same sha256) produces no change entry", async () => {
    await context.start(async () => {
      const h = harness(baseReadSet({ canonicalPages: [{ pageSlug: PAGE_HOME, snapshot: { sha256: SHA_A, size: 100 } }] }))
  ...
  test("diff: a changed hash on an already-present page is 'modified'", async () => {
    await context.start(async () => {
      ...
      expect(result.candidate.changes).toEqual([{ pageSlug: PAGE_HOME, change: "modified", sha256: SHA_B, size: 120 }])
    })
  })
  ...
  test("diff: an expected-absent page (null snapshot) that now exists is 'added'", async () => {
    await context.start(async () => {
      ...
      expect(result.candidate.changes).toEqual([{ pageSlug: PAGE_ABOUT, change: "added", sha256: SHA_B, size: 30 }])
    })
  })
  ...
  test("diff: a page present at admission but absent from the candidate is 'removed'", async () => {
    await context.start(async () => {
      ...
      expect(result.candidate.changes).toEqual([{ pageSlug: PAGE_GONE, change: "removed", sha256: null, size: null }])
    })
  })
  ...
  test("diff never touches file content — only relPath/sha256/size are read", async () => {
    ...
    await context.start(async () => {
      ...
      expect(result.candidate.changes).toEqual([{ pageSlug: PAGE_HOME, change: "modified", sha256: SHA_B, size: 100 }])
    })
  })
})
