...
- `src/gate/model/gate.ts` — the orchestrator (`runGate`): always runs the import scan, page contract, and all five warning lints against the source; `typeCheck`/`checkManifest`/`smokeRender` are optional injected ports. `smokeRender` now has a driving factory (`model/smoke.ts`'s `createSmokeRender`) that builds the port from an injected `SmokeRenderer`, but stays unreachable in a running app until a composition root supplies real adapters for all three ports — none of which exist yet for `typeCheck`/`checkManifest`, and no host-side `SmokeRenderer` adapter exists yet either. `GateInput` carries two further optional fields — `referencedIds` (feeds `dropped-id`) and `listedSlugs` (feeds `unlisted-navigation`) — and each lint skips silently when its field is absent, so the gate still runs standalone with no kernel/manifest behind it
...
- `src/gate/ports/smoke-renderer.ts` — the `SmokeRenderer` port the gate declares, plus its `smokeResultToErrors` result mapper; `src/gate/model/smoke.ts`'s `createSmokeRender` is the driving factory that turns an injected `SmokeRenderer` into the gate's `smokeRender` port (source hash computed locally, parity-pinned by test against the host's own `computeSourceHash`); `src/host/supervisor/model/one-shot.ts` implements the one-shot session a real `SmokeRenderer` needs, but no adapter literally satisfying `SmokeRenderer` exists yet, so nothing calls `createSmokeRender` from a composition root today

**HostSupervisor / design host — `src/host/`:**

- `src/host/protocol/` — the versioned wire codecs: hello, control envelope, frame, strict JSON, and the hard size/geometry limits; `model/embedded-declaration.ts` holds this binary's own `EMBEDDED_RUNTIME_DECLARATION` constant, shared by `main.tsx`'s `_host` branch and (once WP-4 lands) `HostSupervisorDeps.runtimeDeclaration`
...
- `src/host/supervisor/model/spawn-command.ts` — `createHostSpawnCommand`: the compiled-vs-dev `SpawnCommand` builder (`[execPath, "_host", "--stdio"]` vs `[execPath, srcRoot, "_host", "--stdio"]`); not yet called by any composition root
