...
describe("createFakeAgentBackend", () => {
  test("startTurn() returns a run bound to the exact fence, with no events until emitted", async () => {
    ...
    backend.completeRun(fence, { kind: "completed", finalText: "done", usage: null, sessionId: "sess-1" })
    const outcome = await run.outcome
    expect(outcome).toEqual({ kind: "completed", finalText: "done", usage: null, sessionId: "sess-1" })
  })
  ...
  test("healthCheck() defaults to ready, and queueHealth() scripts one degraded probe", async () => {
    ...
    backend.queueHealth({ backendId: "fake-backend", health: { status: "not-installed" }, account: null })
  ...
  test("sessionScope() is a pure deterministic function of its input", () => {
    const backend = createFakeAgentBackend()
    const a = backend.sessionScope({ account: "acct-1", model: "claude", workspaceIdentity: "ws-1" })
    const b = backend.sessionScope({ account: "acct-1", model: "claude", workspaceIdentity: "ws-1" })
    const c = backend.sessionScope({ account: "acct-2", model: "claude", workspaceIdentity: "ws-1" })
    expect(a).toBe(b)
    expect(a).not.toBe(c)
  })
  ...
  test("records startTurn/cancel/healthCheck/sessionScope calls in order", async () => {
    ...
    expect(backend.calls.map((c) => c.method)).toEqual(["startTurn", "cancel", "healthCheck", "sessionScope"])
  })
})
