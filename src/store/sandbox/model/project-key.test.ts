...
describe("computeProjectKey — storage-identity §4 / turn-durability §6.2", () => {
  ...
  test("is the lowercase-hex SHA-256 of root || 0x00 || projectId, byte for byte", () => {
    ...
    const expected = crypto
      ...
      .update(
        Buffer.concat([
          Buffer.from(input.canonicalProjectRoot, "utf8"),
          Buffer.from([0x00]),
          Buffer.from(input.projectId, "utf8"),
        ]),
      )
      .digest("hex");
  ...
  test("differs for the same basename under a different root (no two projects share a parent)", () => {
    const a = computeProjectKey({
      canonicalProjectRoot: "/home/alice/project",
      projectId: PROJECT_ID,
    });
    const b = computeProjectKey({
      canonicalProjectRoot: "/home/bob/project",
      projectId: PROJECT_ID,
    });
    expect(a).not.toBe(b);
  });
