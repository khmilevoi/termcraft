...
import { classifyTransaction, listTransactionIds, nodeRecoveryFsDeps, recoverTransactions } from "./recovery"
...
function replaceOp(target: string, bytes: Uint8Array, payloadId: string, oldImage: TransactionOperation["oldImage"] = { state: "absent" }): TransactionOperation {
  return { index: 0, target, mode: "replace", oldImage, newImage: { state: "file", sha256: sha256Hex(bytes), size: bytes.byteLength }, payloadId }
}

function blankPlan(transactionId: string, operations: readonly TransactionOperation[]): TransactionPlan {
  return { journalVersion: 1, transactionId, kind: "project-mutation", domain: {}, createdAt: TS, operations }
}
 ...
 */
async function crashAtBoundary(safeFs: SafeProjectFs, input: RunTransactionInput, boundary: TransactionBoundary): Promise<void> {
  const deps = {
...
describe("listTransactionIds", () => {
  ...
  test("sorts entries in stable lexical order and skips a non-UUID stray entry", () => {
    fs.mkdirSync(path.join(termcraftDir, "transactions.local", "11111111-1111-7111-8111-111111111112"), { recursive: true })
    fs.mkdirSync(path.join(termcraftDir, "transactions.local", "00000000-0000-7000-8000-000000000001"), { recursive: true })
    fs.mkdirSync(path.join(termcraftDir, "transactions.local", "not-a-uuid"), { recursive: true })
    const safeFs = openSafeFs()
    const deps = nodeRecoveryFsDeps(safeFs)
    expect(listTransactionIds(deps)).toEqual(["00000000-0000-7000-8000-000000000001", "11111111-1111-7111-8111-111111111112"])
  })
})
...
describe("recovery — discard: valid plan and payloads, no intent", () => {
  test("the prepared journal is deleted; targets are untouched by contract", async () => {
    ...
    await crashAtBoundary(safeFs, { mutex, permit, plan, payloads: new Map([[PAYLOAD_A, bytes]]) }, "plan-installed")

    expect(fs.existsSync(path.join(termcraftDir, "transactions.local", TX_ID, "plan.json"))).toBe(true)
    expect(fs.existsSync(path.join(termcraftDir, "transactions.local", TX_ID, "intent.json"))).toBe(false)
  ...
  test("a directory with only a stray payload and no plan.json degrades to discard", async () => {
    ...
    fs.mkdirSync(path.join(termcraftDir, "transactions.local", TX_ID, "payloads"), { recursive: true })
    fs.writeFileSync(path.join(termcraftDir, "transactions.local", TX_ID, "payloads", `${PAYLOAD_A}.bin`), "orphan\n")
...
describe("recovery — roll-forward: valid intent, no valid committed", () => {
  test("'before': target still equals the operation's old image — the planned new image is applied", async () => {
    ...
    const plan = blankPlan(TX_ID, [replaceOp("cache/tx.bin", bytes, PAYLOAD_A)])
    await crashAtBoundary(safeFs, { mutex, permit, plan, payloads: new Map([[PAYLOAD_A, bytes]]) }, "intent-installed")
    ...
    expect(fs.readFileSync(path.join(termcraftDir, "cache", "tx.bin"), "utf8")).toBe("fresh contents\n")
  ...
  test("'after': target already equals the new image (ambiguous ack) — marked applied without rewriting", async () => {
    ...
    const plan = blankPlan(TX_ID, [replaceOp("cache/tx.bin", bytes, PAYLOAD_A)])
    await crashAtBoundary(safeFs, { mutex, permit, plan, payloads: new Map([[PAYLOAD_A, bytes]]) }, "operation-target-applied")

    expect(fs.readFileSync(path.join(termcraftDir, "cache", "tx.bin"), "utf8")).toBe("ambiguous ack contents\n")
    expect(fs.existsSync(path.join(termcraftDir, "transactions.local", TX_ID, "applied", "0000.json"))).toBe(false)
  ...
  test("'empty-append': a fresh JSONL file (beforeLength 0) is fully written", async () => {
    ...
    await crashAtBoundary(safeFs, { mutex, permit, plan, payloads: new Map([[APPEND_PAYLOAD_A, appendBytes]]) }, "intent-installed")
  ...
  test("'complete-without-ack': JSONL already shows the full appended bytes — marked applied without re-appending", async () => {
    ...
    await crashAtBoundary(safeFs, { mutex, permit, plan, payloads: new Map([[APPEND_PAYLOAD_A, appendBytes]]) }, "operation-target-applied")
    ...
    const deps = nodeRecoveryFsDeps(safeFs)
    let appends = 0
    const spiedDeps = { ...deps, appendInPlace: (appendInput: Parameters<typeof deps.appendInPlace>[0]) => { appends += 1; return deps.appendInPlace(appendInput) } }
  ...
  test("'changed-target': a fixed target matching neither old nor new image is a durable recovery conflict, never overwritten", async () => {
    ...
    const plan = blankPlan(TX_ID, [
      { index: 0, target: "cache/tx.bin", mode: "replace", oldImage: { state: "file", sha256: sha256Hex(oldBytes), size: oldBytes.byteLength }, newImage: { state: "file", sha256: sha256Hex(newBytes), size: newBytes.byteLength }, payloadId: PAYLOAD_A },
    ])
    await crashAtBoundary(safeFs, { mutex, permit, plan, payloads: new Map([[PAYLOAD_A, newBytes]]) }, "intent-installed")
    ...
    expect(fs.readFileSync(path.join(termcraftDir, "cache", "tx.bin"), "utf8")).toBe("unrelated hostile content\n")
...
describe("recovery — stop: invalid plan, payload, or marker (journal_corrupt; write nothing)", () => {
  test("'missing-payload': a payload absent from disk is journal_corrupt distinctly", async () => {
    ...
    expect(outcome.ok).toBe(false)
    expect(fs.existsSync(path.join(termcraftDir, "cache", "tx.bin"))).toBe(false)
    expect(fs.existsSync(path.join(termcraftDir, "transactions.local", TX_ID, "conflict.json"))).toBe(false)
  })
  ...
  test("'bad-hash': a payload whose bytes disagree with the declared hash is journal_corrupt distinctly", async () => {
    ...
    const classified = classifyTransaction(deps, TX_ID)
    expect(classified).toBeInstanceOf(JournalCorruptError)
    expect(classified instanceof JournalCorruptError && classified.code).toBe("PAYLOAD_HASH_MISMATCH")
  })
  ...
  test("'corrupt-intent': intent.json naming the wrong plan hash is journal_corrupt distinctly", async () => {
    ...
    fs.writeFileSync(path.join(termcraftDir, intentPath(TX_ID)), JSON.stringify({ planHash: "0".repeat(64) }))
...
describe("recovery — complete: valid committed marker", () => {
  test("recognized complete without comparing targets — legitimate later drift is never treated as a conflict", async () => {
    ...
    const result = await runTransaction(txDeps, { mutex, permit, plan, payloads: new Map([[PAYLOAD_A, bytes]]) })
    ...
    expect(fs.readFileSync(path.join(termcraftDir, "cache", "tx.bin"), "utf8")).toBe("drifted after commit\n")
  })
})
...
describe("recovery — conflict marker exists", () => {
  test("a pre-existing conflict.json is re-presented as the same conflict, with no further write", async () => {
    ...
    fs.mkdirSync(path.join(termcraftDir, "cache"), { recursive: true })
    fs.writeFileSync(path.join(termcraftDir, "cache", "tx.bin"), "already something else entirely\n")

    const txDeps = nodeTransactionFsDeps(safeFs)
    const firstResult = await runTransaction(txDeps, { mutex, permit, plan, payloads: new Map([[PAYLOAD_A, newBytes]]) })
    ...
    expect(outcome.error).toBeInstanceOf(TransactionRecoveryConflictError)
    expect(fs.readFileSync(path.join(termcraftDir, "cache", "tx.bin"), "utf8")).toBe("already something else entirely\n")
  })
})
  ...
  test("resolves everything sorting before a conflict; halts before anything sorting after it", async () => {
    ...
    // ID_CONFLICT: the target is hostile-corrupted before the attempt, so roll-forward writes a real conflict.json.
    {
      ...
      const plan = blankPlan(ID_CONFLICT, [
        { index: 0, target: "cache/conflict.bin", mode: "replace", oldImage: { state: "file", sha256: sha256Hex(oldBytes), size: oldBytes.byteLength }, newImage: { state: "file", sha256: sha256Hex(newBytes), size: newBytes.byteLength }, payloadId: PAYLOAD_A },
      ])
      const result = await runTransaction(txDeps, { mutex, permit, plan, payloads: new Map([[PAYLOAD_A, newBytes]]) })
    ...
    expect(fs.readFileSync(path.join(termcraftDir, "cache", "roll.bin"), "utf8")).toBe("roll-forward payload\n")
    expect(fs.existsSync(path.join(termcraftDir, "transactions.local", ID_ROLL, "committed.json"))).toBe(true)
    expect(fs.readFileSync(path.join(termcraftDir, "cache", "complete.bin"), "utf8")).toBe("already committed\n")
