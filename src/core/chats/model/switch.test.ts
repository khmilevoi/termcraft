...
import { createFakeChatStore, createFakePageStore, createFakePinStore, createFakeProjectStore } from "core/ports/fakes"

import { switchChat } from "./switch"

const FAILURE: FailureDtoV1 = { code: "PERSISTENCE_FAILED", retryable: false, safeMessage: "no such chat", details: {} }
...
describe("switchChat", () => {
  test("selects an existing chat as active and returns a chat.changed payload with no added/updated/removed entries", async () => {
    ...
    expect(result).toEqual({ activeChatId: created.chatId, added: [], updated: [], removedChatIds: [] })
