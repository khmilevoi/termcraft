...
import { WorkspaceChangedDuringSnapshotError, nodeCandidateDeps, snapshotToCandidate } from "./candidate"
...
function countingDeps(): { deps: CandidateDeps; sinksOpened: () => number; bytesWritten: () => number } {
...
describe("snapshotToCandidate — turn-durability §5.4 immutable candidate", () => {
  test("copies the exact allowed inventory while hashing", () => {
    ...
    expect(result.files.map((f) => f.relPath).sort()).toEqual(["RUNTIME.md", "pages.json", "pages/about.tsx", "pages/home.tsx"])
  ...
  test("the recorded hash is the SHA-256 of the copied bytes", () => {
    ...
    const expected = new Bun.CryptoHasher("sha256").update(fs.readFileSync(path.join(workspace, "pages", "home.tsx"))).digest("hex")
  ...
  test("refuses a destination nested inside the source (§5.4 is `outside the workspace`)", () => {
    ...
    const result = snapshotToCandidate({ source: openWorkspace(), destRoot: inside, deps: nodeCandidateDeps() })
...
describe("snapshotToCandidate — a hostile workspace passes zero bytes to any consumer", () => {
  ...
  test("a hardlinked intake file is rejected before any byte is copied", () => {
    ...
    if (!linked) throw new Error("could not plant a hardlink; `mklink /H` needs no elevation and must succeed")
