...
import { JournalCorruptError, JournalTooNewError, TRANSACTIONS_LOCAL_DIR, computePlanHash, decodePlan, encodeCanonicalJson, fileImageSchema, sha256HexSchema, validatePlanPayloads } from "./plan"
...
export interface TransactionFsDeps {
   ...
   */
  readonly appendInPlace: (input: { absPath: string; truncateTo: number; offset: number; bytes: Uint8Array }) => TransactionIoError | undefined
  ...
  return {
    ...
    appendInPlace: (input) => {
      const done = errore.try({
        ...
        catch: (cause) => new TransactionIoError({ operation: "append", path: input.absPath, cause }),
      })
...
export function classifyAppendTail(input: {
  ...
  readonly append: { readonly beforeLength: number; readonly beforePrefixSha256: Sha256Hex; readonly appendedLength: number }
 ...
 */
function ensureDirDurable(deps: Pick<TransactionFsDeps, "ensureDir" | "flushDir">, absDir: string): TransactionIoError | DurabilityError | undefined {
  ...
  for (let cursor = absDir; cursor !== existingAncestor; cursor = path.dirname(cursor)) missing.unshift(cursor)
 ...
 */
function installBytesIfAbsent(deps: TransactionFsDeps, relPath: string, bytes: Uint8Array): SafeFsError | DurabilityError | TransactionIoError | undefined {
...
export function installJsonIfAbsent(deps: TransactionFsDeps, relPath: string, value: unknown): SafeFsError | DurabilityError | TransactionIoError | undefined {
...
const appliedMarkerSchema = z.object({ index: z.number().int().nonnegative(), planHash: sha256HexSchema, realizedImage: fileImageSchema })
const intentSchema = z.object({ planHash: sha256HexSchema })
const committedMarkerSchema = z.object({
  transactionId: z.string(),
  planHash: sha256HexSchema,
  operations: z.array(z.object({ index: z.number().int().nonnegative(), realizedImage: fileImageSchema })),
})

function readJson<T>(deps: TransactionFsDeps, relPath: string, schema: z.ZodType<T>, code: string): SafeFsError | JournalCorruptError | T | null {
  ...
  const parsed = errore.try({
    ...
    catch: (cause) => new JournalCorruptError({ code: `${code}_PARSE`, reason: `${relPath} is not valid JSON`, cause }),
  })
  if (parsed instanceof Error) return parsed
  const result = schema.safeParse(parsed.value)
  if (!result.success) return new JournalCorruptError({ code: `${code}_SHAPE`, reason: `${relPath} has an invalid shape` })
  return result.data
}

/** Read `plan.json` back and validate it (turn-durability §3.3, §4.6). Reusable by recovery (T14). */
export function readPlan(deps: TransactionFsDeps, transactionId: string): SafeFsError | JournalCorruptError | JournalTooNewError | TransactionPlan | null {
...
export function readIntent(deps: TransactionFsDeps, transactionId: string): SafeFsError | JournalCorruptError | TransactionIntent | null {
...
export function readCommitted(deps: TransactionFsDeps, transactionId: string): SafeFsError | JournalCorruptError | CommittedMarker | null {
 ...
 */
export function loadTransactionPayloads(deps: TransactionFsDeps, plan: TransactionPlan): SafeFsError | Map<string, Uint8Array> {
...
type ApplyOutcome = SafeFsError | JournalCorruptError | TransactionIoError | DurabilityError | TransactionRecoveryConflictError | { readonly realizedImage: FileImage }
...
function applyFixedOperation(input: {
  ...
  if (op.mode === "delete") {
    ...
    if (!imagesEqual(after, op.newImage)) {
      return new TransactionRecoveryConflictError({ transactionId, operationIndex: op.index, reason: `deleting ${op.target} did not realize the planned absent image` })
    }
    return { realizedImage: after }
  }

  // replace
  if (op.payloadId === undefined) return new JournalCorruptError({ code: "PAYLOAD_MISSING", reason: `operation ${op.index} is a replace with no payloadId` })
  const bytes = input.payloads.get(op.payloadId)
  if (bytes === undefined) return new JournalCorruptError({ code: "PAYLOAD_MISSING", reason: `operation ${op.index} names payload ${op.payloadId}, which was not prepared` })
  ...
  if (after instanceof Error) return after
  if (!imagesEqual(after, op.newImage)) {
    return new TransactionRecoveryConflictError({ transactionId, operationIndex: op.index, reason: `replacing ${op.target} did not realize the planned new image` })
  }
  return { realizedImage: after }
}
...
function applyAppendOperation(input: {
  ...
  if (append === undefined) return new JournalCorruptError({ code: "APPEND_MISSING", reason: `operation ${op.index} is append-jsonl with no append descriptor` })

  const appendBytes = input.payloads.get(append.appendedPayloadId)
  if (appendBytes === undefined) {
    return new JournalCorruptError({ code: "PAYLOAD_MISSING", reason: `operation ${op.index} names append payload ${append.appendedPayloadId}, which was not prepared` })
  }
  ...
  if (fileExists) {
    const applied = deps.appendInPlace({ absPath, truncateTo: append.beforeLength, offset: append.beforeLength, bytes: appendBytes })
  ...
  if (after instanceof Error) return after
  if (!imagesEqual(after, op.newImage)) {
    return new TransactionRecoveryConflictError({ transactionId, operationIndex: op.index, reason: `appending to ${op.target} did not realize the planned new image` })
  }
  return { realizedImage: after }
}
...
function readAppliedMarker(deps: TransactionFsDeps, transactionId: string, index: number): SafeFsError | JournalCorruptError | AppliedMarker | null {
  return readJson(deps, appliedMarkerPath(transactionId, index), appliedMarkerSchema, "APPLIED_MARKER")
}

function validateExistingMarker(marker: AppliedMarker, op: TransactionOperation, planHash: Sha256Hex): JournalCorruptError | null {
  if (marker.index !== op.index) return new JournalCorruptError({ code: "APPLIED_MARKER_INDEX", reason: `applied marker names operation ${marker.index}, expected ${op.index}` })
  if (marker.planHash !== planHash) return new JournalCorruptError({ code: "APPLIED_MARKER_PLAN_HASH", reason: `applied marker for operation ${op.index} names a different plan hash` })
  ...
  if (!imagesEqual(marker.realizedImage, op.newImage)) {
    return new JournalCorruptError({ code: "APPLIED_MARKER_IMAGE", reason: `applied marker for operation ${op.index} does not match its planned newImage` })
  }
  return null
}

function writeAppliedMarker(deps: TransactionFsDeps, transactionId: string, op: TransactionOperation, planHash: Sha256Hex, realizedImage: FileImage) {
...
function writeConflictMarker(deps: TransactionFsDeps, transactionId: string, operationIndex: number, reason: string) {
...
function verifyRealizedOperations(deps: TransactionFsDeps, plan: TransactionPlan): SafeFsError | TransactionRecoveryConflictError | null {
  for (const op of plan.operations) {
...
function asPostIntentFailure(
  ...
  error: SafeFsError | JournalCorruptError | TransactionIoError | DurabilityError | TransactionRecoveryConflictError,
): JournalCorruptError | TransactionRecoveryConflictError | TransactionIoPendingError {
  if (error instanceof JournalCorruptError || error instanceof TransactionRecoveryConflictError) return error
...
export async function rollForwardTransaction(deps: TransactionFsDeps, input: RollForwardInput) {
  ...
  for (const op of ordered) {
    ...
    if (existingMarker instanceof Error) return asPostIntentFailure(plan.transactionId, existingMarker)
    ...
    const outcome =
      op.mode === "append-jsonl"
        ? applyAppendOperation({ deps, transactionId: plan.transactionId, op, payloads: input.payloads })
        : applyFixedOperation({ deps, transactionId: plan.transactionId, op, payloads: input.payloads })
    if (outcome instanceof Error) {
      if (outcome instanceof TransactionRecoveryConflictError) {
        const marked = writeConflictMarker(deps, plan.transactionId, op.index, String(outcome.reason))
        if (marked instanceof Error) console.warn("transaction: failed to record conflict marker:", marked.message)
        return outcome
      }
    ...
    const markerWritten = writeAppliedMarker(deps, plan.transactionId, op, planHash, outcome.realizedImage)
    if (markerWritten instanceof Error) return asPostIntentFailure(plan.transactionId, markerWritten)
  ...
  const committed: CommittedMarker = { transactionId: plan.transactionId, planHash, operations: realized }
  const wroteCommitted = installJsonIfAbsent(deps, committedPath(plan.transactionId), committed)
  if (wroteCommitted instanceof Error) return asPostIntentFailure(plan.transactionId, wroteCommitted)
...
function writePayloads(deps: TransactionFsDeps, transactionId: string, plan: TransactionPlan, payloads: ReadonlyMap<string, Uint8Array>) {
  for (const op of plan.operations) {
    const payloadId = operationPayloadId(op)
    if (payloadId === undefined) continue
