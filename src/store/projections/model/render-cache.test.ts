...
function cacheOver(memory: ReturnType<typeof memoryFs>, options?: { quotaBytes?: number; isPinned?: (key: ExportRenderKey) => boolean; storeGeneration?: number }) {
...
function entryHash(key: ExportRenderKey): string {
  return crypto.createHash("sha256").update(new TextEncoder().encode(canonicalizeExportRenderKey(key))).digest("hex")
}
...
describe("createRenderCache", () => {
  ...
  ] as const)("changing %s alone is a miss, and a subsequent put rebuilds it into a fresh hit", async (_field, changedKey) => {
    const memory = memoryFs()
    const cache = cacheOver(memory)
    await cache.put(baseEntry)
  ...
  test("a corrupted buffer (checksum mismatch) is a miss", async () => {
    ...
    const envelope = JSON.parse(new TextDecoder().decode(raw)) as { styledFrame: { base64: string } }
