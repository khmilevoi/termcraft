...
// Implementation note: SeedThemeId is deliberately NOT ThemeId, which spec §4.6 widened to
// `string` because a project's theme names live in its own manifest — a `Record<ThemeId, …>`
// would become an index signature and, under this repository's `noUncheckedIndexedAccess: true`,
// make themeTokens() return `TokenMap | undefined`, breaking three call sites outside this
// module for no gain.
/**
...
// Internal — not on the @termcraft/runtime facade. See seedThemeCapability's note below for why.
//
...
// seedThemeCapability, and a page READS it (via `themeCapability()`) and must not write it.
...
// seedThemeCapability; the values come from the project's `design/system/design-system.json`,
// which is inside `treeRoot` and covered by `expectedFiles`, so no protocol change carries them.
//
...
// Internal — not on the @termcraft/runtime facade: an authored page must not be able to repaint
// its own theme.
//
...
// `src/entrypoint/model/create-shell.ts` already uses for `runtime/generated/runtime-dts`.
//
