...
import type { AppendInterruption, JsonlDocument, JsonlRecordIdentity, PreparedAppendPayload, TailClassification } from "../types"
import { JSONL_MAX_PHYSICAL_LINE_BYTES, JSONL_LF, JsonlLineTooLongError, decodeJsonlLine, sha256Hex } from "./line-codec"
...
export function readJsonlDocument<H extends object, R extends JsonlRecordIdentity>(
  ...
  function decodeLine(raw: RawLine, isHeaderLine: boolean): H | R | Error {
    if (raw.bytes === null) {
      return new JsonlLineTooLongError({ measured: raw.length, allowed: JSONL_MAX_PHYSICAL_LINE_BYTES })
    }
    const parsed = decodeJsonlLine(raw.bytes)
    if (parsed instanceof Error) return parsed
    return isHeaderLine ? input.codec.decodeHeader(parsed.value) : input.codec.decodeRecord(parsed.value)
  }
  ...
  function handleLine(raw: RawLine): JsonlMidFileCorruptionError | null {
    ...
    if (seenRecordIds.has(record.recordId)) {
      return new JsonlMidFileCorruptionError({ path: input.path, offset: raw.start, reason: `duplicate recordId ${record.recordId}` })
    }
  ...
  if (integrity instanceof Error) {
    return new JsonlMidFileCorruptionError({ path: input.path, offset: state.validPrefixBytes, reason: integrity.message })
  }
  ...
  if (proven !== null) return document({ kind: "transaction-proven-append-interruption", append: proven }, false)
...
function classifyPreparedAppend(input: {
  ...
  if (input.trailingTotal === descriptor.appendedLength) return { ...base, state: "complete", truncateTo: null }
