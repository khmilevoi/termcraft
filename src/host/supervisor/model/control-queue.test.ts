...
function env(kind: string, extra: Record<string, JsonValue> = {}): ControlEnvelope {
  mid += 1
  return { protocolVersion: 1, kind, sessionId: ID.sessionId, nonce: ID.nonce, messageId: String(mid), body: extra }
}
const discrete = (kind: string, extra?: Record<string, JsonValue>) => ({ kind, envelope: env(kind, extra) })
const coalescible = (kind: string, extra?: Record<string, JsonValue>) => ({ kind, envelope: env(kind, extra) })
