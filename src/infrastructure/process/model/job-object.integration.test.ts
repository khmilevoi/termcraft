...
test.if(win)("kill-on-close: close() WITHOUT terminate() kills the adopted child (Spike I crash-safety net)", async () => {
...
test.if(win)("adopt() on a nonexistent pid returns a ProcessTreeError from the OpenProcess-null branch", () => {
...
test.if(win)("activeProcesses()/terminate()/adopt() refuse with a ProcessTreeError after close()", async () => {
...
test.if(win)("close() is idempotent: a second close() never throws and the tree stays refused", () => {
