...
function classifyProject(components: readonly string[]): ManagedNamespace | null {
  ...
  if (components.length === 1) {
    if (first === "project.toml" || first === "workspace.local.toml" || first === ".gitignore") return "project-config"
    return null
  }
  if (first === "chats") {
    if (components.length !== 2 || second === undefined) return null
...
export function createLimitBudget(rootKind: ManagedRootKind): LimitBudget {
  ...
  function checkBytes(input: {
    ...
    if (input.fileBytes > nsLimit.perFileBytes) {
      return exceeded(`${input.namespace} per-file`, input.relPath, input.fileBytes, nsLimit.perFileBytes)
    }
    if (nsLimit.aggregateBytes !== null && input.namespaceTotal > nsLimit.aggregateBytes) {
      return exceeded(`${input.namespace} aggregate`, input.relPath, input.namespaceTotal, nsLimit.aggregateBytes)
    }
    if (rootLimit.totalBytes !== null && input.rootTotal > rootLimit.totalBytes) {
      return exceeded(`${rootKind} tree aggregate`, input.relPath, input.rootTotal, rootLimit.totalBytes)
    }
    return null
  }
