...
import { createFakeDiagnosticsCache, createFakePageMetaCache, createFakeRenderCache } from "./projections"

const FAILURE: FailureDtoV1 = { code: "PERSISTENCE_FAILED", retryable: false, safeMessage: "cache unreadable", details: {} }
...
describe("createFakePageMetaCache", () => {
  ...
  test("put() then get() round-trips the exact entry", async () => {
    const cache = createFakePageMetaCache()
    const key = { pageSlug: "home", sourceHash: "a".repeat(64), extractorVersion: 1 }
    const entry = { key, meta: { kitApiVersion: 1, title: "Home", minSize: { w: 80, h: 24 }, theme: "default" } }
  ...
  test("a key with a different sourceHash is a distinct cache slot (a miss)", async () => {
    ...
    await cache.put({ key: key1, meta: { kitApiVersion: 1, title: "Home", minSize: { w: 80, h: 24 }, theme: "default" } })
...
describe("createFakeDiagnosticsCache", () => {
  test("put() then get() round-trips", async () => {
    ...
    const entry = { key, schemaVersion: 1, provenance: "gate" as const, observedAt: "2024-01-01T00:00:00.000Z", diagnostics: [] }
    await cache.put(entry)
    expect(await cache.get(key)).toEqual(entry)
  })
})
...
describe("createFakeRenderCache", () => {
  test("put() then get() round-trips opaque render bytes", async () => {
    ...
    const entry = { key, styledFrame: new Uint8Array([1]), textFrame: new Uint8Array([2]), layout: new Uint8Array([3]) }
    await cache.put(entry)
    expect(await cache.get(key)).toEqual(entry)
  })
  ...
  test("records get/put calls in order across all three caches independently", async () => {
    ...
    await pageMeta.put({ key, meta: { kitApiVersion: 1, title: "Home", minSize: { w: 80, h: 24 }, theme: "default" } })
