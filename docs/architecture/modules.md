...
| **Agent gateway** | Backends over vendors' official TypeScript SDKs: start, stream, confirmed process-tree cancel, health-check; normalization into backend-neutral events; per-chat session checkpoints; per-backend confinement to a unique turn workspace; fencing by turn, attempt, and lease nonce. No `agent/` module exists yet, but three pieces it will consume already do: `entities/turn` defines the `AgentEvent` normalization target and the `TurnFence` fencing token, and Project store already provides the unique per-turn workspace and a nonce shaped for the fence — nothing wires them into a live backend yet. |
...
Four of the seven components are real, tested code today (Runtime facade, Gate,
HostSupervisor, Project store); UI shell, Kernel, and Agent gateway have no module yet
(`ui/`, `core/`, `agent/` do not exist under `src/`) and stay anchored to spec only.
...
- `docs/architecture/code-structure.md` — where `core/ports/` will sit and which already-built adapters (store, gate, host) it will inject

**Agent gateway — no `agent/` module yet; the vocabulary and infrastructure it will consume are already landed elsewhere:**

- `docs/superpowers/specs/2026-07-13-termcraft-design.md` — §6.1 backend abstraction and confinement, §6.2 turn protocol
- `src/entities/turn/types.ts` — `AgentEvent` (the normalized event union a backend maps vendor output into) and `TurnFence` (`turnId`/`attempt`/`leaseNonce`); no backend consumes either yet
- `src/store/sandbox/model/staging-store.ts` — the unique per-turn workspace (`{userStateRoot}/sandboxes/<projectKey>/turns/<turnId>/workspace/`) a backend will be confined to; the store creates it, nothing yet spawns an agent into it
