...
describe("control envelope", () => {
  ...
  test("rejects a wrong protocolVersion", () => {
    expect(decodeControlEnvelope(encode({ ...base, protocolVersion: 2 }))).toBeInstanceOf(
      ProtocolError,
    );
  });

  test("rejects a messageId that is not a decimal uint string", () => {
    expect(decodeControlEnvelope(encode({ ...base, messageId: "01" }))).toBeInstanceOf(
      ProtocolError,
    );
  });
  ...
  test("round-trips an envelope with responseTo", () => {
    const response: ControlEnvelope = {
      ...base,
      kind: "set-mode-result",
      messageId: "9",
      responseTo: "7",
      body: {},
    };
  ...
  test("rejects a bad requestId", () => {
    expect(decodeControlEnvelope(encode({ ...request, requestId: "0" }))).toBeInstanceOf(
      ProtocolError,
    );
  });

  test("rejects a bad responseTo", () => {
    expect(decodeControlEnvelope(encode({ ...base, responseTo: "x" }))).toBeInstanceOf(
      ProtocolError,
    );
  });
