...
1. Eight components, each with a single responsibility:
   ...
   | **Kernel** | The only decision-maker: commands in, events out, the agent-turn lifecycle (staging assembly → turn → gate → apply), the turn-time locks, the design host's lifecycle; Git history/Restore decisions and scoped-commit planning, confirmation-bound `restoreActionId` creation, pre-write freshness revalidation, in-memory `record-pending` recovery, and project-write mutex coordination through the `GitHistory` and `GitCommitter` ports |
...
8. Git interaction does not share the agent-turn lock wholesale. In v1 the commit split-button remains available while an agent turn runs; during that turn scoped commit execution contends through the project-write mutex only with short final apply, not network streaming, validation, or Gate. Restore remains locked during the turn; outside agent turns, its ordered canonical-page replacement and active-chat record append use the same mutex. At confirmation the Kernel creates a unique `restoreActionId` bound to the page, full commit id, and confirmed pre-Restore source hash. Initial execution under the mutex revalidates that hash, Gates the exact snapshot, replaces the canonical source once, then appends a record containing `restoreActionId`, page, and full commit id. These writes are ordered but not one crash-atomic transaction. If append fails after replacement, the restored page stays active in explicit `record-pending`. Append-only **Retry record** checks the active chat for `restoreActionId` under the same mutex and either accepts the existing record or appends exactly one; it never repeats Gate or source replacement and does not revalidate the pre-Restore hash, so ambiguous acknowledgement is idempotent and subsequent page drift does not invalidate the audit fact. A new Restore requires a new confirmation and `restoreActionId`. In v1 `record-pending` is in memory only: after a process crash or restart the source remains on disk, no automatic audit-record recovery runs, the UI refreshes from disk and Git status, and the missing record may require manual or operator diagnosis. While a historical snapshot is selected, Send, Tweaks, pin creation, pin reopening, and pin-status changes are disabled until the user returns to Current design; existing current pins are only overlaid where their ids resolve.
