...
import { PROJECT_MANIFEST_FILENAME, decodeProjectManifest, encodeProjectManifest } from "store/toml"
import type { ProjectManifest } from "store/toml"
import { buildMigrationTransaction, createWriteMutex, nodeTransactionFsDeps } from "store/transaction"
import type { FileImage, TransactionOperation, TransactionWrapperDeps } from "store/transaction"

import type { CreateBackupInput } from "../types"
import {
...
import type { BackupStoreDeps } from "../types"
...
function baseInput(overrides: Partial<CreateBackupInput> & { readonly files: CreateBackupInput["files"] }): CreateBackupInput {
  return {
    projectId: uuidv7(),
...
function makeFakeDeps(overrides: {
  ...
  const deps: BackupStoreDeps = {
    ...
    mkdirNew: (absDir) => {
      if (overrides.failMkdirNewFor?.(absDir) === true) return new MigrationIoError({ operation: "mkdir-new", path: absDir, cause: new Error("simulated") })
      if (dirs.has(absDir)) return new MigrationIoError({ operation: "mkdir-new", path: absDir, cause: new Error("EEXIST") })
      dirs.add(absDir)
      return undefined
    },
    durableWrite: (absPath, bytes) => {
      if (overrides.failDurableWriteFor?.(absPath) === true) return new DurableWriteError({ path: absPath, step: "write" })
    ...
    readFile: (absPath) => {
      ...
      if (original === undefined) return new MigrationIoError({ operation: "read", path: absPath, cause: new Error("ENOENT") })
...
describe("createBackupStore — the §12 verified-backup protocol", () => {
  test("copies every file, writes the manifest, verifies each copy, then writes VERIFIED last", async () => {
    ...
    expect(result.backupDir).toBe(backupActionDir(USER_STATE_ROOT, input.projectId, input.migrationActionId))
    ...
    expect(result.manifest.files).toEqual([
      { relPath: "project.toml", byteLength: bytesA.byteLength, sha256: sha256Hex(bytesA), sourceFormat: "project.toml@1", backupTime: new Date(TS).toISOString() },
      {
    ...
    const manifestPath = backupManifestPath(USER_STATE_ROOT, input.projectId, input.migrationActionId)
    const verifiedPath = backupVerifiedMarkerPath(USER_STATE_ROOT, input.projectId, input.migrationActionId)
  ...
  test("completes the same unconditional protocol whether or not the project is Git-tracked (storage-identity §12: Git presence never waives backup)", async () => {
    ...
    const store = createBackupStore(deps)
    const input = baseInput({ files: [{ relPath: "project.toml", bytes: bytesOf("v1"), sourceFormat: "project.toml@1" }] })

    const result = await store.createBackup(input)
    if (result instanceof Error) throw new Error(`unexpected error: ${result.message}`)
    expect(files.has(backupVerifiedMarkerPath(USER_STATE_ROOT, input.projectId, input.migrationActionId))).toBe(true)
  })
  ...
  test("a create-new collision on the migrationActionId directory fails before any copy", async () => {
    ...
    if (first instanceof Error) throw new Error(`unexpected error on first backup: ${first.message}`)
  ...
  test("a copy failure fails at the copy step and never writes the manifest or VERIFIED", async () => {
    ...
    const { deps, files } = makeFakeDeps({ failDurableWriteFor: (absPath) => absPath.endsWith(relPath) })
    const store = createBackupStore(deps)
    const input = baseInput({ files: [{ relPath, bytes: bytesOf("v1"), sourceFormat: "project.toml@1" }] })
    ...
    expect(files.has(backupManifestPath(USER_STATE_ROOT, input.projectId, input.migrationActionId))).toBe(false)
    expect(files.has(backupVerifiedMarkerPath(USER_STATE_ROOT, input.projectId, input.migrationActionId))).toBe(false)
  })

  test("a manifest write failure never writes VERIFIED", async () => {
    const { deps, files } = makeFakeDeps({ failDurableWriteFor: (absPath) => absPath.endsWith(BACKUP_MANIFEST_FILENAME) })
    const store = createBackupStore(deps)
    const input = baseInput({ files: [{ relPath: "project.toml", bytes: bytesOf("v1"), sourceFormat: "project.toml@1" }] })
  ...
  test("a reopened copy that disagrees with the source and the manifest fails verification and never writes VERIFIED", async () => {
    const relPath = "project.toml"
    const { deps, files } = makeFakeDeps({
      corruptReadFor: (absPath) => (absPath.endsWith(relPath) ? bytesOf("corrupted on disk") : bytesOf("unused")),
    })
    const store = createBackupStore(deps)
    const input = baseInput({ files: [{ relPath, bytes: bytesOf("v1"), sourceFormat: "project.toml@1" }] })
  ...
  test("path helpers compose {userStateRoot}/backups/{projectId}/{migrationActionId}", () => {
    ...
    expect(backupActionDir(USER_STATE_ROOT, projectId, migrationActionId)).toBe(path.join(USER_STATE_ROOT, "backups", projectId, migrationActionId))
    ...
    expect(backupVerifiedMarkerPath(USER_STATE_ROOT, projectId, migrationActionId)).toBe(path.join(USER_STATE_ROOT, "backups", projectId, migrationActionId, "VERIFIED"))
  })
})
...
describe("synthetic end-to-end migration", () => {
  ...
  function seedOldManifest(): Uint8Array {
    const manifest: ProjectManifest = { formatVersion: 1, projectId: PROJECT_ID, name: "old-name", createdAt: TS, targetStack: "generic", pages: [] }
  ...
  function stalePrecondition(expected: Uint8Array) {
    return (): MigrationStaleError | null => {
      if (sha256Hex(currentManifestBytes()) !== sha256Hex(expected)) return new MigrationStaleError({ part: PROJECT_MANIFEST_FILENAME })
      return null
    }
  }
  ...
  test("happy path: verified backup, transform, then MigrationTransaction commits the new file", async () => {
    ...
    const backup = await createBackupStore(nodeBackupStoreDeps(userStateRoot, CLOCK)).createBackup({
      ...
      termcraftVersion: "0.0.0-test",
      files: [{ relPath: PROJECT_MANIFEST_FILENAME, bytes: sourceBytes, sourceFormat: "project.toml@1" }],
    })
    if (backup instanceof Error) throw new Error(`unexpected backup error: ${backup.message}`)
    expect(fs.existsSync(backupVerifiedMarkerPath(userStateRoot, PROJECT_ID, migrationActionId))).toBe(true)
    expect(fs.existsSync(backupManifestPath(userStateRoot, PROJECT_ID, migrationActionId))).toBe(true)
    ...
    const newManifest: ProjectManifest = { formatVersion: 1, projectId: PROJECT_ID, name: "migrated-name", createdAt: TS, targetStack: "generic", pages: [] }
    const newBytes = bytesOf(encodeProjectManifest(newManifest))
    const payloadId = uuidv7()
    const oldImage: FileImage = { state: "file", sha256: sha256Hex(sourceBytes), size: sourceBytes.byteLength }
  ...
  test("source drift during backup yields migration_stale and leaves the source untouched", async () => {
    ...
    // pre-intent re-check.
    const driftedManifest: ProjectManifest = { formatVersion: 1, projectId: PROJECT_ID, name: "drifted-by-someone-else", createdAt: TS, targetStack: "generic", pages: [] }
