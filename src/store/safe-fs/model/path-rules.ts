...
function validateComponent(component: string): PathRuleError | null {
  if (component === "") return reject("EMPTY_COMPONENT", "a managed path may not contain an empty component")
  if (component === "." || component === "..") {
    return reject("DOT_COMPONENT", `\`${component}\` is not a legal managed path component`)
  }
  if (component.includes(":")) {
    return reject("COLON", `\`${component}\` contains a colon (an alternate data stream is never a managed leaf)`)
  }
  ...
  if (Buffer.byteLength(component, "utf8") > MAX_COMPONENT_BYTES) {
    return reject("COMPONENT_TOO_LONG", `\`${component}\` exceeds ${MAX_COMPONENT_BYTES} UTF-8 bytes`)
  }
  return null
}
...
export function validateRelativePath(raw: string): PathRuleError | readonly string[] {
  ...
  if (hasControlCharacter(raw)) return reject("CONTROL_CHAR", "a managed path may not contain NUL or control characters")
  ...
  if (raw.startsWith("\\\\") || raw.startsWith("//")) return reject("UNC", "a UNC path is never a managed path")
  if (raw.startsWith("/")) return reject("ABSOLUTE_POSIX", "a POSIX absolute path is never a managed path")
  if (/^[A-Za-z]:/.test(raw)) {
    return reject("DRIVE_PATH", "a drive-rooted or drive-relative Windows path is never a managed path")
  }
  if (raw.includes("\\")) return reject("BACKSLASH", "canonical managed paths use `/` separators only")
  ...
  if (components.length > MAX_PATH_COMPONENTS) {
    return reject("TOO_MANY_COMPONENTS", `the path has more than ${MAX_PATH_COMPONENTS} components`)
  }
 ...
 */
export function detectNameCollisions(directory: string, names: readonly string[]): NameCollisionError | null {
