...
export function nodeCandidateDeps(): CandidateDeps {
  ...
  return {
    ...
    openSource(absPath) {
      const fd = errore.try({
        try: () => fs.openSync(absPath, "r"),
        catch: (cause) => accessError("open", absPath, cause),
      });
    ...
    createNewSink(absPath) {
      const fd = errore.try({
        try: () => fs.openSync(absPath, "wx"),
        catch: (cause) => accessError("create", absPath, cause),
      });
      if (fd instanceof Error) return fd;
      return {
        write: (chunk) => {
...
function enumerateDirectory(input: {
  ...
  const absDir =
    input.relDir === ""
      ? input.source.realPath
      : path.join(input.source.realPath, ...input.relDir.split("/"));
...
function streamThroughHandle(input: {
  ...
  for (;;) {
    ...
    const overflow = input.budget.observeBytes({
...
export function snapshotToCandidate(input: {
  ...
  const nested =
    input.destRoot === input.source.realPath ||
    input.destRoot.startsWith(input.source.realPath + path.sep);
  if (nested) {
    return new WorkspaceChangedDuringSnapshotError({
      path: input.destRoot,
  ...
  const entries = enumerateDirectory({
    source: input.source,
    relDir: "",
    deps: input.deps,
    budget,
  ...
  const duplicate = detectDuplicateIdentity(
    entries.map((entry) => ({ path: entry.relPath, stat: entry.stat })),
  );
