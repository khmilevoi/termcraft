...
// VERBATIM LIFT of `src/agent/types.ts` (decision C1: "Only `agent/types.ts` is genuinely
// liftable verbatim" — every other store/host port needed a narrow, core-owned redraw
// because their source files import several sibling submodules a verbatim copy would
// have to follow across the module boundary). `agent/types.ts` imports only
// `entities/turn`, so its exact text is reproduced here unchanged (down to comments);
// `agent/types.ts` itself carries the matching note that it is "a port lifted verbatim
// into `core/ports/` in phase 6".
//
// CONFORMANCE (decision C2 — "adapters MAY import `core/ports/*` TYPE-ONLY to implement
// a port. One definition, both sides."): the direction of that type-only import is
// `agent -> core/ports`, never the reverse. `core` imports no other module (code-structure
// §7; this slice's structural rule repeats it verbatim: "no import from store/host/gate/
// agent anywhere under src/core/**"), so the compile-time check that
// `createProductionClaudeBackend()`'s return type is assignable to the `AgentBackend`
// declared below cannot run from inside this file or anywhere under `core/`. It belongs in
// `agent/` (e.g. `agent/types.test.ts`), which may write:
//
// ```ts
// import type { AgentBackend as CoreAgentBackend, AssertConforms } from "core/ports"
// import { createProductionClaudeBackend } from "./claude"
// type _Check = AssertConforms<CoreAgentBackend, ReturnType<typeof createProductionClaudeBackend>>
// ```
//
// `AssertConforms` is declared in `core/ports/index.ts`. That one-line addition is left to
// whichever agent owns `agent/*` in this slice — this file's own file list is `core/ports/`
// only, and touching `agent/types.test.ts` from here would risk clobbering concurrent
// sibling work (binding rule 5).
