...
flowchart TD
    ...
    blocked -. "the only route to Home that names its reason" .-> home
...
- `ShellLaunchV1` (`src/entrypoint/types.ts`) now carries exactly one field, `existing: boolean`
  ...
  computed and, before fix-bundle §2.4, threw away. `resolveShellLaunch(existing)` is a straight
  pass-through now, not a fold. It used to also fold in a separate CONTENT probe —
...
- `run-app.ts`'s `runApp` dispatches `project.open` itself, once, right after the shutdown path
  ...
  "rejected"`) — both `console.error` the cause AND call `root.abandonStartupOpen()`
  (`UiRootHandle`, `src/ui/app/model/root.tsx`), which flips `UiLocalState.startupOpenPending`
  false so the Workspace does not sit "opening" forever. What that recovery actually SHOWS the
  user is narrower than "the failure now has a surface" might suggest: neither branch reaches the
  Kernel's own `kernel.project.blockOpen` (that only fires from INSIDE the ready sequence, step 11
  below), so `ProjectMirror.openFailure` stays `null` and `HomeOpenFailurePanel` — gated on
  `openFailure !== null` — renders nothing. The user lands on a bare Home, able to retry
  (`home-submit` still dispatches `project.open` for it, below), but told no reason; the
  `console.error` diagnostic is real but reaches only the trace file (tracing on) or a held buffer
  flushed at quit (tracing off) — never the screen.
...
7. On first submit, termcraft creates portable `project.toml`, machine-local
   ...
   blocked partway through the ready sequence, with the reason named on screen (step 11),
   or abandoned before the Kernel ever admitted it, with none (Gap D above) — the second
   of those two abandon branches (`result.status === "rejected"`) IS a `CommandResultV1`
   carrying a Kernel guard code, so that dispatch DID reach the Kernel and was refused by
   it; only the first (`result instanceof Error`) genuinely never got there. The
...
11. Failure branch: an open that cannot finish. Nine distinct steps of the ready
    ...
    Home *with a reason on screen*: a startup open that never reaches this far — the
    dispatch itself rejecting, or the Kernel refusing it outright — also drops back to
    Home (Gap D's two abandon branches), but with no `openFailure` set and so no panel;
    every other existing-project open mounts and fills the Workspace directly.
