...
describe("tokenize — the over-fire proof, re-measured on every run", () => {
  test("every one of the repository's own .ts/.tsx files tokenizes, with zero refusals", () => {
    ...
    // `runtime/ui/line-number.tsx` and its own `line-number.test.tsx`, taking it to 951.
    ...
    // — update BOTH this number and the count quoted in `lexer.oracle.test.ts` when the corpus
    // grows.
    // The project-design-systems P5 plan (tagless renderables) adds `runtime/ui/renderable-tags.ts`,
    // four wrapper modules with a test beside each, and `renderable-tags.test.tsx`, taking it to 974.
    // — update BOTH this number and the count quoted in `lexer.oracle.test.ts` when the corpus
    // grows.
    // manifest-and-gate (P2) adds the `entities/design-system` module (`types.ts`, `index.ts`,
    // `model/manifest.ts`, `model/components.ts` and their tests), `gate/model/design-system.ts`
    ...
    // — update BOTH this number and the count quoted in `lexer.oracle.test.ts` when the corpus
    // grows.
    // The project-design-systems P4 plan (integration spine) adds 16 files, taking it to 1035.
    expect(files.length).toBe(1035);
  });
