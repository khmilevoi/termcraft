...
describe("runGate (§6.3 pipeline)", () => {
  test("a clean page with no injected stages passes and carries its descriptor", async () => {
    ...
    expect(result.descriptor).toEqual({ slug: SLUG, meta: { kitApiVersion: 1, title: "Dashboard", minSize: { w: 80, h: 24 }, theme: "dark-default" } })
  })

  test("a forbidden import fails the candidate with an import-kind error", async () => {
    const result = await runGate({ source: `import { x } from "lodash"\n${cleanSource}`, slug: SLUG })
  ...
  test("an injected type-check stage contributes fatal errors", async () => {
    const typeError: GateError = { kind: "type", code: "TYPE_ERROR", message: "Type 'string' is not assignable to 'number'" }
    const result = await runGate({ source: cleanSource, slug: SLUG }, { typeCheck: () => [typeError] })
    expect(result.ok).toBe(false)
  ...
  test("manifest + smoke stages only run when the source scans + type check are clean", async () => {
    ...
    await runGate({ source: badSource, slug: SLUG }, {
      smokeRender: () => {
        smokeRan = true
        return []
      },
  ...
  test("the smoke stage runs and contributes errors for a clean-source candidate", async () => {
    const smokeError: GateError = { kind: "smoke", code: "DESIGN_RENDER_FAILED", message: "render threw" }
    const result = await runGate({ source: cleanSource, slug: SLUG }, { smokeRender: () => [smokeError] })
