...
5. Switching resumes the target chat's stored session id.
...
6. Context usage: the status-bar segment renders token usage the backend reports in its event stream; a backend that reports nothing hides the segment. termcraft never compacts a conversation itself — starting a new chat is the designer's context management.
