 ...
 */
export function checkManagedLeaf(
  path: string,
  stat: SafeFsStat,
): LeafRejectedError | UnsafeHardlinkError | null {
  if (stat.isSymbolicLink)
    return new LeafRejectedError({
      path,
      reason: "it is a symbolic link, junction, or reparse point",
    });
  if (stat.isSpecial)
    return new LeafRejectedError({ path, reason: "it is a FIFO, socket, or device" });
  if (stat.isDirectory)
    return new LeafRejectedError({
      path,
      reason: "it is a directory where a regular file is required",
    });
  if (!stat.isFile)
    return new LeafRejectedError({ path, reason: "it is not an ordinary regular file" });
  if (stat.nlink !== 1)
    return new UnsafeHardlinkError({ path, reason: `link count is ${stat.nlink}, not exactly 1` });
  return null;
}
...
export function checkManagedDirectory(path: string, stat: SafeFsStat): LeafRejectedError | null {
  if (stat.isSymbolicLink)
    return new LeafRejectedError({
      path,
      reason: "a managed directory may not be a reparse point",
    });
 ...
 */
export function checkIdentityUnchanged(
  path: string,
  before: SafeFsStat,
  after: SafeFsStat,
  ...
  if (before.dev !== after.dev || before.ino !== after.ino) {
    return new IdentityChangedError({
      path,
      reason: `file identity moved from ${identityKey(before)} to ${identityKey(after)}`,
    });
  }
  if (before.nlink !== after.nlink) {
    return new IdentityChangedError({
      path,
      reason: `link count moved from ${before.nlink} to ${after.nlink}`,
    });
  }
  if (before.size !== after.size) {
    return new IdentityChangedError({
      path,
      reason: `size moved from ${before.size} to ${after.size}`,
    });
  }
  if (before.mtimeNs !== after.mtimeNs) {
    return new IdentityChangedError({
      path,
      reason: `mtime moved from ${before.mtimeNs} to ${after.mtimeNs}`,
    });
  }
  return null;
}
