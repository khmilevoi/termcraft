...
describe("openManagedRoot", () => {
  ...
  test("refuses a root that is not a directory", () => {
    ...
    expect(openManagedRoot({ kind: "project", path: filePath, deps })).toBeInstanceOf(LeafRejectedError)
  })

  test("refuses a root that does not exist", () => {
    expect(openManagedRoot({ kind: "project", path: path.join(scratch, "nope"), deps })).toBeInstanceOf(FsAccessError)
  })
})
...
describe("resolveManagedPath", () => {
  ...
  test("resolves a path whose tail does not exist yet (a create target)", () => {
    const resolved = resolveManagedPath({ root: openRoot(), relPath: "pages/about/page.tsx", deps })
    expect(resolved).not.toBeInstanceOf(Error)
  })

  test("rejects a path that violates the §5.1 grammar before touching the filesystem", () => {
    expect(resolveManagedPath({ root: openRoot(), relPath: "../escape.txt", deps })).toBeInstanceOf(PathRuleError)
  })
})
...
describe("planted junction escape (Spike F step 5)", () => {
  ...
  test("a naive path.join would have missed the escape, and SafeProjectFs does not", () => {
    if (!planted) throw new Error("could not plant a junction; `mklink /J` needs no elevation and must succeed")
    ...
    expect(assertWithinRoot({ rootReal: fs.realpathSync(root), absPath: naiveJoined, deps })).toBeInstanceOf(PathEscapeError)
  })
  ...
  test("the junction itself is rejected as a managed directory component", () => {
    ...
    expect(resolveManagedPath({ root: openRoot(), relPath: `${linkName}/x.tsx`, deps })).toBeInstanceOf(
      ReparsePointRejectedError,
    )
  })
...
describe("assertWithinRoot", () => {
  test("accepts the root itself and any real descendant", () => {
    ...
    expect(assertWithinRoot({ rootReal, absPath: path.join(rootReal, "pages", "home"), deps })).toBeNull()
  })
...
describe("SafeProjectFs", () => {
  ...
  test("refuses every §5.1 path class without touching the filesystem", () => {
    const safeFs = createSafeProjectFs(openRoot(), deps)
    for (const hostile of ["", "..", "/etc/passwd", "C:/Windows", "a\\b", "a:b", "a/./b", "a//b", "con", "a."]) {
