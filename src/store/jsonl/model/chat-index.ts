...
async function computeWindowHashes(
  ...
  const finalBytes = wholePrefixFitsOneWindow
    ? firstBytes
    : await source.readRange({ start: finalStart, end: validPrefixBytes });
...
async function flushEntries(
  ...
  for (let at = 0; at < entries.length; at += CHAT_INDEX_PAGE_SIZE) {
    ...
    if (wrote instanceof Error)
      return new ChatIndexIoError({
        chatId,
        reason: `page ${pageIndex} write failed: ${wrote.message}`,
        cause: wrote,
      });
    directory.push({
      pageIndex,
      entryCount: slice.length,
      firstOffsetStart: first.offsetStart,
      lastOffsetEnd: last.offsetEnd,
      checksum,
    });
    pageIndex += 1;
  }
...
async function readVerifiedPage(
  ...
  pageIndex: number,
  pages: ChatIndexPageStore,
): Promise<ChatIndexIoError | ChatIndexPage> {
  const page = await pages.readPage(chatId, pageIndex);
  if (page instanceof Error)
    return new ChatIndexIoError({
      chatId,
      reason: `page ${pageIndex} read failed: ${page.message}`,
      cause: page,
    });
  if (page === null)
    return new ChatIndexIoError({
      chatId,
      reason: `page ${pageIndex} is missing from the page store`,
    });
  if (checksumEntries(page.entries) !== page.checksum)
    return new ChatIndexIoError({ chatId, reason: `page ${pageIndex} failed its checksum` });
  return page;
}
...
async function rebuildFromByteZero(input: {
  ...
  if (windows instanceof Error)
    return new ChatIndexIoError({ chatId: input.chatId, reason: windows.message, cause: windows });
 ...
 */
function scanSuffixLines(
  suffix: Uint8Array,
  baseOffset: number,
): { entries: readonly ChatIndexEntry[]; newValidPrefixBytes: number } | null {
  const entries: ChatIndexEntry[] = [];
...
export async function updateChatIndex(
  previous: ChatIndexState,
  input: UpdateChatIndexInput,
): Promise<ChatIndexError | ChatIndexState> {
  const rebuild = () =>
    rebuildFromByteZero({
  ...
  if (windows instanceof Error)
    return new ChatIndexIoError({
      chatId: previous.chatId,
      reason: windows.message,
      cause: windows,
    });
  if (windows.first !== previous.firstWindowSha256) return rebuild();
  if (windows.final !== previous.finalWindowSha256) return rebuild();

  const suffix = await input.source.readRange({
    start: previous.validPrefixBytes,
    end: input.totalBytes,
  });
  if (suffix instanceof Error)
    return new ChatIndexIoError({ chatId: previous.chatId, reason: suffix.message, cause: suffix });
  ...
  const appended = await flushEntries(
    previous.chatId,
    scanned.entries,
  ...
  const finalWindowBytes = await input.source.readRange({
    start: finalWindowStart,
    end: scanned.newValidPrefixBytes,
  });
  if (finalWindowBytes instanceof Error)
    return new ChatIndexIoError({
      chatId: previous.chatId,
      reason: finalWindowBytes.message,
      cause: finalWindowBytes,
    });
...
async function selectBackward(input: {
  ...
  outer: for (let pi = input.directory.length - 1; pi >= 0; pi -= 1) {
    ...
    if (
      input.beforeOffsetExclusive !== null &&
      dirEntry.firstOffsetStart >= input.beforeOffsetExclusive
    )
      continue;
    ...
    for (let ei = page.entries.length - 1; ei >= 0; ei -= 1) {
      ...
      if (input.beforeOffsetExclusive !== null && entry.offsetStart >= input.beforeOffsetExclusive)
        continue;
...
async function materializeRecords(
  ...
  source: CanonicalRangeReader,
  entries: readonly ChatIndexEntry[],
): Promise<ChatIndexIoError | readonly ChatRecord[]> {
  ...
  if (bytes instanceof Error)
    return new ChatIndexIoError({ chatId, reason: bytes.message, cause: bytes });

  const records: ChatRecord[] = [];
  for (const entry of entries) {
    const line = bytes.subarray(
      entry.offsetStart - first.offsetStart,
      entry.offsetEnd - first.offsetStart,
    );
    const decoded = decodeChatRecordLine(line);
    if (decoded instanceof Error)
      return new ChatIndexIoError({
        chatId,
        reason: `canonical record ${entry.recordId} no longer decodes: ${decoded.message}`,
        cause: decoded,
      });
...
function toPrevCursor(
  state: ChatIndexState,
  picked: { entries: readonly ChatIndexEntry[]; reachedBeginning: boolean },
): PageCursor | null {
...
export async function loadChatIndexBefore(
  ...
  if (cursor.generation !== state.generation)
    return new ChatIndexCursorStaleError({ chatId: state.chatId });
