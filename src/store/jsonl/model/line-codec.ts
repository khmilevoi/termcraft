...
export function decodeJsonlLine(physicalLine: Uint8Array): JsonlLineError | JsonlLineValue {
  if (physicalLine.byteLength > JSONL_MAX_PHYSICAL_LINE_BYTES) {
    return new JsonlLineTooLongError({
      measured: physicalLine.byteLength,
      allowed: JSONL_MAX_PHYSICAL_LINE_BYTES,
    });
  }
  ...
  if (content.includes(CR))
    return new JsonlLineFormatError({ reason: "carriage return in a line" });
 ...
 */
export function encodeChatHeaderLine(
  header: ChatHeader,
): ChatDecodeError | JsonlLineError | Uint8Array {
...
export function encodeChatRecordLine(
  record: ChatRecord,
): ChatDecodeError | JsonlLineError | Uint8Array {
...
export function encodeCommentsHeaderLine(
  header: CommentsHeader,
): PinDecodeError | JsonlLineError | Uint8Array {
...
export function decodeChatHeaderLine(
  physicalLine: Uint8Array,
): JsonlLineError | ChatDecodeError | ChatHeader {
  const parsed = decodeJsonlLine(physicalLine);
  if (parsed instanceof Error) return parsed;
  return decodeChatHeader(parsed.value);
}

export function decodeChatRecordLine(
  physicalLine: Uint8Array,
): JsonlLineError | ChatDecodeError | ChatRecord {
  const parsed = decodeJsonlLine(physicalLine);
  if (parsed instanceof Error) return parsed;
  return decodeChatRecord(parsed.value);
}

export function decodeCommentsHeaderLine(
  physicalLine: Uint8Array,
): JsonlLineError | PinDecodeError | CommentsHeader {
  const parsed = decodeJsonlLine(physicalLine);
  if (parsed instanceof Error) return parsed;
  return decodeCommentsHeader(parsed.value);
}

export function decodePinEventLine(
  physicalLine: Uint8Array,
): JsonlLineError | PinDecodeError | PinEvent {
  const parsed = decodeJsonlLine(physicalLine);
  if (parsed instanceof Error) return parsed;
