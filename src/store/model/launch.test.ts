...
import {
  DataFormatTooNewError as MigrationDataFormatTooNewError,
...
import type { ChildPayload, FileImage, TransactionBoundary, TransactionOperation, TransactionPlan } from "store/transaction"
...
describe("openProject — existing-project launch ordering (storage-identity §14.1 / turn-durability §12)", () => {
  test("recover transactions runs before schemas: a leftover intended-but-uncommitted transaction is rolled forward and the manifest reflects the ROLLED-FORWARD state, not the stale one", async () => {
    ...
    const created = await store.createProject({ root: projectRoot, name: "Original", targetStack: "generic" })
    ...
    if (originalManifest instanceof Error) throw new Error(`fixture bug: ${originalManifest.message}`)
    await created.close()
    ...
    const oldBytes = new Uint8Array(fs.readFileSync(absOf(termcraftDir, PROJECT_MANIFEST_FILENAME)))
    const oldImage: FileImage = { state: "file", sha256: sha256Hex(oldBytes), size: oldBytes.byteLength }
    ...
    const plan: TransactionPlan = {
      ...
      createdAt: TS,
      operations: [{ index: 0, target: PROJECT_MANIFEST_FILENAME, mode: "replace", oldImage, newImage: { state: "file", sha256: sha256Hex(newBytes), size: newBytes.byteLength }, payloadId }],
    }
    ...
    writeManaged(termcraftDir, intentPath(transactionId), encodeCanonicalJson({ planHash: computePlanHash(plan) }))
    ...
    try {
      ...
      expect(reopened.recovery).toEqual({ ok: true, recovered: 1, discarded: 0, alreadyComplete: 1 })
  ...
  test("the migrations gate fires before schemas: a too-new format_version is reported as DataFormatTooNewError (store/migration), not the schemas step's own ManifestTooNewError", async () => {
    ...
    const bumped = fs.readFileSync(manifestPath, "utf8").replace("format_version = 1", "format_version = 2")
  ...
  test("a hard journal_too_new transaction blocks the whole launch, and the lease is still released for a later attempt", async () => {
    ...
    const termcraftDir = path.join(projectRoot, ".termcraft")
    const transactionId = uuidv7()
    writeManaged(termcraftDir, planPath(transactionId), new TextEncoder().encode(JSON.stringify({ journalVersion: 2 })))
  ...
  test("a transactions.local/format.json naming a journalVersion newer than the binary blocks the whole launch — even when a transaction directory has no readable plan.json (blocker finding #1)", async () => {
    ...
    fs.writeFileSync(absOf(termcraftDir, "transactions.local/format.json"), JSON.stringify({ journalVersion: 2 }))
    ...
    fs.writeFileSync(path.join(futureTxDir, "unknown-v2-marker.json"), JSON.stringify({ someFutureShape: true }))
  ...
  test("the orphan turn scan runs as part of open: a user turn with no terminal record is terminalized, and the result is visible on the opened chat", async () => {
    ...
    const termcraftDir = path.join(projectRoot, ".termcraft")
    const turnId = uuidv7()
    const userRecord: ChatUserRecord = { kind: "user", recordId: uuidv7(), turnId, text: "hello", ts: TS }
    ...
    try {
      ...
      const terminal = tail.records.find((record) => record.kind === "system:error" && record.turnId === turnId)
  ...
  test("load stores + open: every port on the returned handle is present and minimally functional", async () => {
    ...
    const created = await store.createProject({ root: projectRoot, name: "Smoke", targetStack: "generic" })
    if (created instanceof Error) throw new Error(`fixture bug: ${created.message}`)
    await created.close()
    ...
    try {
      ...
      expect(reopened.recovery).toEqual({ ok: true, recovered: 0, discarded: 0, alreadyComplete: 1 })
      ...
      expect(await reopened.projections.pageMetaGet({ pageSlug: "home", sourceHash: sha256Hex(new Uint8Array()), extractorVersion: 1 })).toBeNull()
  ...
  test("createProject still refuses an existing layout even after a successful openProject/close cycle", async () => {
    ...
    const created = await store.createProject({ root: projectRoot, name: "Once", targetStack: "generic" })
    if (created instanceof Error) throw new Error(`fixture bug: ${created.message}`)
    await created.close()
    ...
    const recreated = await store.createProject({ root: projectRoot, name: "Twice", targetStack: "generic" })
...
describe("identity checks (storage-identity §5.2)", () => {
  test("ChatStore.open refuses a chat whose header names a different chatId than its filename", async () => {
    ...
    const tamperedHeader = encodeChatHeaderLine({ kind: "chat", formatVersion: 1, projectId: manifest.projectId, chatId: otherChatId, createdAt: TS })
  ...
  test("ChatStore.open refuses a chat whose header names a different projectId than project.toml", async () => {
    ...
    const tamperedHeader = encodeChatHeaderLine({ kind: "chat", formatVersion: 1, projectId: otherProjectId, chatId, createdAt: TS })
    if (tamperedHeader instanceof Error) throw new Error(`fixture bug: ${tamperedHeader.message}`)
    fs.writeFileSync(absOf(termcraftDir, chatJsonlPath(chatId)), tamperedHeader)
  ...
  test("the orphan turn scan skips (never mutates) a chat file whose header identity does not match its filename — the exact scenario a misplaced/copied chat log produces", async () => {
    ...
    const headerLine = encodeChatHeaderLine({ kind: "chat", formatVersion: 1, projectId: manifest.projectId, chatId: realChatId, createdAt: TS })
    if (headerLine instanceof Error) throw new Error(`fixture bug: ${headerLine.message}`)
    const orphanTurnId = uuidv7()
    const userRecord: ChatUserRecord = { kind: "user", recordId: uuidv7(), turnId: orphanTurnId, text: "hello", ts: TS }
    const userLine = encodeChatRecordLine(userRecord)
    if (userLine instanceof Error) throw new Error(`fixture bug: ${userLine.message}`)
    ...
    try {
      ...
      expect(fs.readFileSync(absOf(termcraftDir, chatJsonlPath(misplacedFilename)))).toEqual(misplacedBytes)
    } finally {
      await reopened.close()
    }
  }, 20_000)
  ...
  test("PinStore.fold refuses a comments log whose header names a different projectId or pageSlug than its containing project/directory", async () => {
    ...
    const wrongProjectHeader = encodeCommentsHeaderLine({ kind: "pins", formatVersion: 1, projectId: uuidv7(), pageSlug })
    if (wrongProjectHeader instanceof Error) throw new Error(`fixture bug: ${wrongProjectHeader.message}`)
    ...
    if (reopenedForProjectCase instanceof Error) throw new Error(`openProject failed: ${reopenedForProjectCase.message}`)
    ...
    const wrongSlugHeader = encodeCommentsHeaderLine({ kind: "pins", formatVersion: 1, projectId: manifest.projectId, pageSlug: otherSlug })
    if (wrongSlugHeader instanceof Error) throw new Error(`fixture bug: ${wrongSlugHeader.message}`)
    ...
    if (reopenedForSlugCase instanceof Error) throw new Error(`openProject failed: ${reopenedForSlugCase.message}`)
...
describe("chat index persistence (projections §7.1, major finding #3)", () => {
  test("the chat index is durably persisted to cache/chat-index/ and survives a full project close/reopen", async () => {
    ...
    for (let i = 0; i < 5; i += 1) {
      const record: ChatUserRecord = { kind: "user", recordId: uuidv7(), turnId: uuidv7(), text: `message ${i}`, ts: TS }
    ...
    try {
      ...
      expect(tail1.records.filter((r) => r.kind === "user").map((r) => r.recordId)).toEqual(recordIds)
    } finally {
      await first.close()
    }
    ...
    const persistedState = JSON.parse(fs.readFileSync(cacheStatePath, "utf8")) as { chatId: string; generation: number }
    ...
    try {
      ...
      expect(tail2.records.filter((r) => r.kind === "user").map((r) => r.recordId)).toEqual(recordIds)
    } finally {
      await second.close()
    }
  }, 20_000)
  ...
  test("re-opening after new records were appended takes the incremental update path (generation stays 1) rather than a fresh rebuild", async () => {
    ...
    const firstRecord: ChatUserRecord = { kind: "user", recordId: uuidv7(), turnId: uuidv7(), text: "first", ts: TS }
    ...
    const cacheStatePath = path.join(termcraftDir, "cache", "chat-index", chatId, "state.json")
    const stateAfterFirst = JSON.parse(fs.readFileSync(cacheStatePath, "utf8")) as { generation: number }
    ...
    try {
      const secondRecord: ChatUserRecord = { kind: "user", recordId: uuidv7(), turnId: uuidv7(), text: "second", ts: TS }
      ...
      const tail2 = await handle2.loadTail()
      if (tail2 instanceof Error) throw new Error(`fixture bug: ${tail2.message}`)
      expect(tail2.records.filter((r) => r.kind === "user").map((r) => r.recordId)).toEqual([firstRecord.recordId, secondRecord.recordId])

      const stateAfterSecond = JSON.parse(fs.readFileSync(cacheStatePath, "utf8")) as { generation: number }
  ...
  test("a chat spanning more than one scan chunk (>1 MiB) still builds and reads correctly through the disk-backed cold-build path", async () => {
    ...
    for (let i = 0; i < 1500; i += 1) {
      const record: ChatUserRecord = { kind: "user", recordId: uuidv7(), turnId: sharedTurnId, text: "x".repeat(700), ts: TS }
      recordIds.push(record.recordId)
      const line = encodeChatRecordLine(record)
      if (line instanceof Error) throw new Error(`fixture bug: ${line.message}`)
      chunks.push(Buffer.from(line))
    }
    const terminalLine = encodeChatRecordLine({ kind: "agent", recordId: uuidv7(), turnId: sharedTurnId, text: "done", changedPages: [], warnings: [], ts: TS })
    ...
    try {
      ...
      expect(tail.records.filter((r) => r.kind === "user").map((r) => r.recordId)).toEqual(recordIds.slice(-199))
    } finally {
      await reopened.close()
    }
  }, 20_000)
...
function oneFileReplacePlan(termcraftDir: string, transactionId: string): PlanFixture {
  const oldBytes = bytesOf(encodeProjectManifest({ formatVersion: 1, projectId: uuidv7(), name: "Old", createdAt: TS, targetStack: "generic", pages: [] }))
  writeManaged(termcraftDir, PROJECT_MANIFEST_FILENAME, oldBytes)
  const newBytes = bytesOf(encodeProjectManifest({ formatVersion: 1, projectId: uuidv7(), name: "New", createdAt: TS, targetStack: "generic", pages: [] }))
  const payloadId = uuidv7()
  ...
  return {
    plan: { journalVersion: 1, transactionId, kind: "project-mutation", domain: { mutationKind: "title-edit" }, createdAt: TS, operations },
...
function multiPageFinalizationPlan(transactionId: string): PlanFixture {
  ...
  const operations: TransactionOperation[] = [
    { index: 0, target: canonicalPagePath(slugA), mode: "replace", oldImage: { state: "absent" }, newImage: { state: "file", sha256: sha256Hex(bytesA), size: bytesA.byteLength }, payloadId: payloadA },
    { index: 1, target: canonicalPagePath(slugB), mode: "replace", oldImage: { state: "absent" }, newImage: { state: "file", sha256: sha256Hex(bytesB), size: bytesB.byteLength }, payloadId: payloadB },
  ]
  return {
    plan: { journalVersion: 1, transactionId, kind: "turn", domain: { turnId: uuidv7(), phase: "finalization" }, createdAt: TS, operations },
    payloads: [
...
function chatAppendPlan(termcraftDir: string, transactionId: string): PlanFixture {
  const chatId = uuidv7()
  const headerLine = encodeChatHeaderLine({ kind: "chat", formatVersion: 1, projectId: uuidv7(), chatId, createdAt: TS })
  if (headerLine instanceof Error) throw new Error(`fixture bug: ${headerLine.message}`)
  ...
  const record: ChatUserRecord = { kind: "user", recordId: uuidv7(), turnId: uuidv7(), text: "hi", ts: TS }
  const appended = buildChatAppend({ before, records: [record], domainIdentity: record.turnId, deps: APPEND_DEPS })
  if (appended instanceof Error) throw new Error(`fixture bug: ${appended.message}`)
  const after = computeAfterImage({ chunks: [headerLine], beforeLength: before.length, append: appended.bytes })
  ...
  return {
    plan: { journalVersion: 1, transactionId, kind: "turn", domain: { turnId: record.turnId, phase: "admission" }, createdAt: TS, operations },
    payloads: [{ payloadId: appended.descriptor.appendedPayloadId, bytesBase64: base64Of(appended.bytes) }],
  }
}
...
function pinAppendPlan(termcraftDir: string, transactionId: string): PlanFixture {
  const pageSlug = slug("gamma")
  const headerLine = encodeCommentsHeaderLine({ kind: "pins", formatVersion: 1, projectId: uuidv7(), pageSlug })
  if (headerLine instanceof Error) throw new Error(`fixture bug: ${headerLine.message}`)
  writeManaged(termcraftDir, pageCommentsPath(pageSlug), headerLine)

  const before: AppendBase = { length: headerLine.byteLength, prefixSha256: sha256Hex(headerLine) }
  const event: PinCreatedEvent = { kind: "pin:created", recordId: uuidv7(), pinId: uuidv7(), element: "button-1", fx: 0.5, fy: 0.5, text: "looks off", ts: TS }
  ...
  return {
    plan: { journalVersion: 1, transactionId, kind: "project-mutation", domain: { mutationKind: "pin-event" }, createdAt: TS, operations },
    payloads: [{ payloadId: appended.descriptor.appendedPayloadId, bytesBase64: base64Of(appended.bytes) }],
  }
}
...
function localStateUpdatePlan(termcraftDir: string, transactionId: string): PlanFixture {
  ...
  const newBytes = bytesOf(encodeWorkspaceLocalState({ ...defaultWorkspaceLocalState(), activePageSlug: slug("home") }))
  const payloadId = uuidv7()
  ...
  return {
    plan: { journalVersion: 1, transactionId, kind: "project-mutation", domain: { mutationKind: "local-state-write" }, createdAt: TS, operations },
    payloads: [{ payloadId, bytesBase64: base64Of(newBytes) }],
  }
}
...
function exportPublishPlan(transactionId: string): PlanFixture {
  ...
  const operations: TransactionOperation[] = [
    { index: 0, target: `export/generations/${generationId}/frame.bin`, mode: "replace", oldImage: { state: "absent" }, newImage: { state: "file", sha256: sha256Hex(artifactBytes), size: artifactBytes.byteLength }, payloadId: artifactPayload },
    { index: 1, target: "export/current.json", mode: "replace", oldImage: { state: "absent" }, newImage: { state: "file", sha256: sha256Hex(currentBytes), size: currentBytes.byteLength }, payloadId: currentPayload },
  ]
  return {
    plan: { journalVersion: 1, transactionId, kind: "export-publish", domain: { generationId }, createdAt: TS, operations },
    payloads: [
...
function migrationPlan(transactionId: string): PlanFixture {
  const bytes = bytesOf(encodeProjectManifest({ formatVersion: 1, projectId: uuidv7(), name: "Migrated", createdAt: TS, targetStack: "generic", pages: [] }))
  const payloadId = uuidv7()
  const operations: TransactionOperation[] = [
    { index: 0, target: PROJECT_MANIFEST_FILENAME, mode: "replace", oldImage: { state: "absent" }, newImage: { state: "file", sha256: sha256Hex(bytes), size: bytes.byteLength }, payloadId },
  ]
  return {
    plan: { journalVersion: 1, transactionId, kind: "migration", domain: { migrationPlanId: uuidv7(), migrationActionId: uuidv7() }, createdAt: TS, operations },
    payloads: [{ payloadId, bytesBase64: base64Of(bytes) }],
  }
}

const SWEPT_BOUNDARIES: readonly TransactionBoundary[] = ["intent-installed", "operation-target-applied", "committed-installed"]

const SWEEP_CASES: ReadonlyArray<{ readonly name: string; readonly build: (termcraftDir: string) => PlanFixture }> = [
