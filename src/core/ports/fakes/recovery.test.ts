...
const FAILURE: FailureDtoV1 = { code: "TRANSACTION_RECOVERY_CONFLICT", retryable: false, safeMessage: "conflicting transaction", details: {} }
...
describe("createFakeRecoveryService", () => {
  test("recover() defaults to a clean, empty pass", async () => {
    ...
    expect(await service.recover()).toEqual({ ok: true, recovered: 0, discarded: 0, alreadyComplete: 0 })
  })
  ...
  test("classify() honors a configured classification per transactionId", async () => {
    const service = createFakeRecoveryService({ classifications: new Map([["tx-1", { kind: "conflict", transactionId: "tx-1", reason: "manifest drift" }]]) })
    const result = await service.classify("tx-1")
  ...
  test("records calls in order", async () => {
    ...
    expect(service.calls.map((c) => c.method)).toEqual(["recover", "classify", "hasIntentRecord", "scanOrphanTurns"])
  })
})
