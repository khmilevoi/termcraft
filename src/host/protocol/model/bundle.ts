...
import { CONTROL_PAYLOAD_LIMIT_BYTES, DATA_PAYLOAD_LIMIT_BYTES } from "infrastructure/framing";
...
export const runtimeDeclarationBundleSchema = z
  ...
  .superRefine((val, ctx) => {
    if (!val.supportedKitApiVersions.includes(val.currentKitApiVersion)) {
      ctx.addIssue({
        code: "custom",
        message: "supportedKitApiVersions must contain currentKitApiVersion",
      });
    }
  });
