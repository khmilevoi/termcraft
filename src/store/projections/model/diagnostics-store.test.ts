...
function storeOver(memory: ReturnType<typeof memoryFs>, options?: { quotaBytes?: number; isPinned?: (key: DiagnosticsKey) => boolean; storeGeneration?: number }) {
...
const baseEntry: DiagnosticsEntry = {
  ...
  diagnostics: [{ code: "TS2304", severity: "error", message: "Cannot find name 'x'.", range: { line: 3, column: 5 } }],
}
...
describe("createDiagnosticsStore", () => {
  ...
  test("no chat id anywhere in the key or entry shape", () => {
    ...
    expect(Object.keys(entry)).toEqual(["key", "schemaVersion", "provenance", "observedAt", "diagnostics"])
  })
  ...
  test("quota eviction removes the least-recently-written unpinned entry once the quota is exceeded", async () => {
    ...
    const padded = (key: DiagnosticsKey): DiagnosticsEntry => ({
      ...baseEntry,
      key,
      diagnostics: [{ code: "PAD", severity: "info", message: "x".repeat(Math.floor(quotaBytes / 3)) }],
    })
