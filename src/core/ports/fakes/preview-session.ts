...
function createFrameChannel(): { push(frame: PreviewFrameV1): void; close(): void; iterable: AsyncIterable<PreviewFrameV1> } {
      ...
      [Symbol.asyncIterator]() {
        return {
          next(): Promise<IteratorResult<PreviewFrameV1>> {
            ...
            if (closed) return Promise.resolve({ value: undefined as unknown as PreviewFrameV1, done: true })
...
export type PreviewSessionCall =
  ...
  | { readonly method: "setTheme"; readonly theme: string; readonly capabilities: TerminalCapabilitiesV1 }
...
export function createFakePreviewSession(
  ...
  options?: { readonly mode?: "preview" | "historical"; readonly interactionMode?: InteractionModeV1 },
): FakePreviewSession {
  ...
  function emitFrame(frame: PreviewFrameV1): void {
    if (closed) {
      console.warn(`fakes/preview-session: emitFrame after close() for session ${identity.sessionId}, ignored`)
      return
    }
    channel.push(frame)
  }
  ...
  async function setTheme(theme: string, capabilities: TerminalCapabilitiesV1): Promise<FailureDtoV1 | undefined> {
